...
- [ ] Add new CLI subcommand `goblin workspace is-indexed [PATH]` that checks if a path (or any ancestor) is already registered as a workspace
- [ ] Implement in `crates/goblin_main/src/cli.rs` - add `IsIndexed` variant to `WorkspaceCommand` enum
- [ ] Add handler in `crates/goblin_main/src/ui.rs` that calls existing `is_indexed()` service method
...
- [ ] Before running sync, call `$_GOBLIN_BIN workspace is-indexed "$workspace_path"`
...
- [ ] Consider adding a helper message when users first cd into a new directory that suggests running `goblin workspace sync` to enable auto-sync
