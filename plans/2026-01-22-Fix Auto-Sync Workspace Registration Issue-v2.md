...
**Issue:** [#2288](https://github.com/antinomyhq/goblin/issues/2288)

**Problem:** The zsh plugin automatically syncs workspaces in the background on every directory change, causing unintended parent directories to be registered as workspaces. When users run `goblin workspace list`, ancestor directories appear as "Current" instead of the actual working directory.

**Root Cause:** `_goblin_start_background_sync()` in `shell-plugin/lib/helpers.zsh:114-131` unconditionally runs `goblin workspace sync` without checking if the workspace or its ancestors are already registered.
...
- [ ] Add `--porcelain` flag to `workspace info` command in `crates/goblin_main/src/cli.rs:262-266`
- [ ] Update `on_workspace_info()` in `crates/goblin_main/src/ui.rs:3328-3389` to handle porcelain mode
...
- [ ] Modify `_goblin_start_background_sync()` in `shell-plugin/lib/helpers.zsh:114-131`
- [ ] Before running sync, call `$_GOBLIN_BIN workspace info "$workspace_path" --porcelain 2>/dev/null`
...
- [ ] Add debug logging (when `GOBLIN_DEBUG=true`) to indicate when sync is skipped
- [ ] Ensure the check runs silently (stderr redirect already in place)

### Phase 3: Handle Initial Workspace Registration

- [ ] Ensure manual `goblin workspace sync` still works for first-time registration
- [ ] Update shell-plugin documentation to clarify the auto-sync behavior (only syncs already-registered workspaces)
- [ ] Consider adding a one-time message when users navigate to unindexed directories suggesting `goblin workspace sync`

### Phase 4: Update Workspace List Display (Optional Enhancement)

- [ ] Enhance `on_list_workspaces()` in `crates/goblin_main/src/ui.rs:3266-3311` to distinguish between exact match and ancestor match
...
- [ ] Run `goblin workspace sync` once
- [ ] Run `cd /Users/username/Documents/Projects/goblin` (subdirectory)
- [ ] Trigger zsh accept-line (press Enter on any command)
- [ ] Run `goblin workspace list`
- [ ] Verify: `/Users/username/Documents/Projects/goblin` is incorrectly registered as a workspace

### After Fix
- [ ] Clean database and repeat above steps
- [ ] Run `cd /Users/username/Documents/Projects/goblin`
...
- [ ] Run explicit `goblin workspace sync` in subdirectory
...
- [ ] Test `goblin workspace info` with `--porcelain` flag returns correct exit codes
- [ ] Test behavior in deeply nested directories (should find closest ancestor)
- [ ] Test behavior when `GOBLIN_SYNC_ENABLED=false` (should still respect flag)
...
**Mitigation:** Users who rely on automatic workspace creation will need to run `goblin workspace sync` once per workspace. This is a one-time migration cost for better UX long-term.
...
Create a dedicated `goblin workspace is-indexed` command instead of extending `info`.
...
- Zsh plugin already has access to `$_GOBLIN_BIN` for CLI calls
...
- [ ] Document `GOBLIN_SYNC_ENABLED` environment variable
