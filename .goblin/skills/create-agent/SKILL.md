---
name: create-agent
description: Create new agents for the code-goblin application. Agents are stored as .md files in the <cwd>/.goblin/agents directory with YAML frontmatter (id, title, description, reasoning, tools, user_prompt) and markdown body containing agent instructions. Use when users need to add new agents, modify existing agents, or understand the agent file structure.
---
{{{{raw}}}}
# Create Agents

Create and manage agents for the code-goblin application. Agents are specialized AI assistants with specific capabilities, tools, and behaviors.

## File Location

**CRITICAL**: All agent files must be created in the `<cwd>/.goblin/agents` directory, where `<cwd>` is the current working directory of your code-goblin project.

- **Directory**: `<cwd>/.goblin/agents`
- **File format**: `{agent-id}.md`
- **Example**: If your project is at `/home/user/my-project`, agents go in `/home/user/my-project/.goblin/agents/`

This is the only location where goblin will discover and load custom agents.
...
You are Goblin, an expert software engineering assistant...
...
Create the file in the `<cwd>/.goblin/agents` directory with the format: `{agent-id}.md`

**IMPORTANT**: The file MUST be in `<cwd>/.goblin/agents` where `<cwd>` is your current working directory. Agents placed anywhere else will not be discovered by goblin.
...
**Strictly Advisory**: You cannot perform implementation tasks. If asked, offer to switch to an implementation agent like Goblin.
```
...
### File Structure
- [ ] File is in the `<cwd>/.goblin/agents` directory (CRITICAL)
- [ ] Filename matches agent ID (e.g., `goblin.md` for `id: "goblin"`)
...
```markdown
You are Goblin, an expert software engineering assistant designed to help users with programming tasks, file operations, and software development processes.
```
...
### File Location
- **Directory**: `<cwd>/.goblin/agents` (where `<cwd>` is current working directory)
- **Format**: `{agent-id}.md`
- **CRITICAL**: Agents MUST be in this exact location to be discovered by goblin
...
1. **Verify the file location**: Ensure the file is in `<cwd>/.goblin/agents` directory (CRITICAL - agents anywhere else will not be found)
