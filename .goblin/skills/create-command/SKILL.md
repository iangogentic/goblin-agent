---
name: create-command
description: Create new commands for the code-goblin application. Commands are stored as .md files in the <cwd>/.goblin/commands directory with YAML frontmatter (name, description) and markdown body containing command steps. Use when users need to add new commands, modify existing commands, or understand the command file structure. Supports special command tags like <lint> and <test> for automated workflows.
---

# Create Commands

Create and manage commands for the code-goblin application. Commands are modular workflows that can be invoked to perform specific tasks.

## File Location

**CRITICAL**: All command files must be created in the `<cwd>/.goblin/commands` directory, where `<cwd>` is the current working directory of your code-goblin project.

- **Directory**: `<cwd>/.goblin/commands`
- **File format**: `{command-name}.md`
- **Example**: If your project is at `/home/user/my-project`, commands go in `/home/user/my-project/.goblin/commands/`

This is the only location where goblin will discover and load custom commands.
...
Create the file in the `<cwd>/.goblin/commands` directory with the format: `{command-name}.md`

**IMPORTANT**: The file MUST be in `<cwd>/.goblin/commands` where `<cwd>` is your current working directory. Commands placed anywhere else will not be discovered by goblin.
...
- [ ] File is in the `<cwd>/.goblin/commands` directory (CRITICAL)
...
- [ ] **Command is recognized by goblin**: Run `goblin list command --custom` (or `goblin list cmd`) and verify your command appears in the list
...
- **Directory**: `<cwd>/.goblin/commands` (where `<cwd>` is current working directory)
- **Format**: `{command-name}.md`
- **CRITICAL**: Commands MUST be in this exact location to be discovered by goblin
...
4. **Goblin Recognition Test**: Verify the command is recognized by goblin
...
1. **Verify the file location**: Ensure the file is in `<cwd>/.goblin/commands` directory (CRITICAL - commands anywhere else will not be found)
2. Check YAML frontmatter is valid (use `---` delimiters)
3. Ensure the command name matches the filename (without .md)
4. **Verify the command is recognized by goblin**:
...
- **File location**: File MUST be in `<cwd>/.goblin/commands` directory (this is the most common issue)
