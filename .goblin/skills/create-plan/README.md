...
**Usage:**
```bash
./.goblin/skills/create-plan/validate-plan.sh plans/2025-11-27-example-v1.md
```
...
./.goblin/skills/create-plan/validate-all-plans.sh

# Validate plans in custom directory
./.goblin/skills/create-plan/validate-all-plans.sh path/to/plans
```
...
if git diff --cached --name-only | grep -q "^plans/.*\.md$"; then
    ...
    ./.goblin/skills/create-plan/validate-all-plans.sh plans/
    exit $?
fi
```
...
```yaml
- name: Validate Plans
  run: ./.goblin/skills/create-plan/validate-all-plans.sh plans/
```
