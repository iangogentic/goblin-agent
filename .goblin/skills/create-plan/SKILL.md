...
```bash
./.goblin/skills/create-plan/validate-plan.sh plans/{YYYY-MM-DD}-{task-name}-v{N}.md
```
...
- **ALWAYS validate the plan** using `./.goblin/skills/create-plan/validate-plan.sh` after creation
...
- Use `filepath:line` format for file references (e.g., `crates/goblin_repo/src/provider.rs:45`)
