---
name: debug-cli
description: Use when users need to debug, modify, or extend the code-goblin application's CLI commands, argument parsing, or CLI behavior. This includes adding new commands, fixing CLI bugs, updating command options, or troubleshooting CLI-related issues.
---

# CLI Debug Skill

This skill provides a systematic workflow for debugging and verifying changes to the goblin CLI application.

## Core Principles

1. **Always get latest docs first**: Run `--help` to see current commands and options
2. **Use `-p` for testing**: Test goblin by giving it tasks with the `-p` flag
...
./target/debug/goblin [COMMAND] --help

# Subcommand help
./target/debug/goblin [COMMAND] [SUBCOMMAND] --help
```

### 3. Test with `-p` Flag

Use the `-p` flag to give goblin a task to complete without interactive mode:

```bash
# Test with a simple prompt
./target/debug/goblin -p "create a hello world rust program"

# Test with specific functionality
./target/debug/goblin -p "read the README.md file and summarize it"

# Test with complex tasks
./target/debug/goblin -p "analyze the code structure and suggest improvements"
```
...
./target/debug/goblin conversation dump <conversation-id>

# Export as HTML for human-readable format (creates: YYYY-MM-DD_HH-MM-SS-dump.html)
./target/debug/goblin conversation dump --html <conversation-id>

# Use dumped JSON to reproduce issues
./target/debug/goblin --conversation 2025-11-23_12-28-52-dump.json
```
...
./target/debug/goblin conversation clone <source-conversation-id>

# This creates a new conversation ID - use that for testing
./target/debug/goblin --conversation-id <new-cloned-id>
...
cargo build
./target/debug/goblin --help  # Verify new command appears
./target/debug/goblin new-command --help  # Check command docs
./target/debug/goblin -p "test the new feature"
```
...
./target/debug/goblin conversation dump <bug-conversation-id>

# 2. Clone it for testing (preserves original)
./target/debug/goblin conversation clone <bug-conversation-id>

# 3. Reproduce with the cloned conversation
./target/debug/goblin --conversation-id <cloned-id> -p "reproduce the issue"

# 4. After fix, verify with new clone
./target/debug/goblin conversation clone <bug-conversation-id>
./target/debug/goblin --conversation-id <new-clone-id> -p "verify fix"
```
...
./target/debug/goblin command

# Test with invalid input
./target/debug/goblin -p "invalid task with special chars: <>|&"

# Test with boundary values
./target/debug/goblin -p "create a file with a very long name..."
```
...
./target/debug/goblin conversation dump <id>
...
./target/debug/goblin conversation dump --html <id>

# 4. Test modified prompts
./target/debug/goblin -p "your optimized prompt here"
```
...
1. **Build**: `cargo build`
2. **Docs**: `./target/debug/goblin --help` (verify documentation)
3. **Test**: `./target/debug/goblin -p "relevant task"`
4. **Verify**: Check output matches expectations

### Debugging a Bug Report

1. **Clone**: `./target/debug/goblin conversation clone <source-id>`
2. **Build**: `cargo build` (with potential fixes)
3. **Test**: `./target/debug/goblin --conversation-id <cloned-id> -p "reproduce"`
...
cargo build
./target/debug/goblin --help  # Always check docs first
./target/debug/goblin -p "your test task"
...
# Clone and test bug
./target/debug/goblin conversation clone <source-id>
./target/debug/goblin --conversation-id <cloned-id> -p "reproduce bug"

# Debug prompts with jq (use actual filename)
cat 2025-11-23_12-28-52-dump.json | jq '.messages[] | {role, content}'

# Test with verbose output
./target/debug/goblin --verbose -p "test task"
```
...
- **Use relative paths**: Binary is at `./target/debug/goblin` from project root
