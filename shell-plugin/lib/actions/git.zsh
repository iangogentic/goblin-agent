...
function _Goblin_action_commit() {
    ...
    if [[ -n "$additional_context" ]]; then
        commit_message=$(FORCE_COLOR=true CLICOLOR_FORCE=1 $_GOBLIN_BIN commit --max-diff "$_GOBLIN_MAX_COMMIT_DIFF" $additional_context)
    else
        commit_message=$(FORCE_COLOR=true CLICOLOR_FORCE=1 $_GOBLIN_BIN commit --max-diff "$_GOBLIN_MAX_COMMIT_DIFF")
    fi
    _Goblin_reset
}
...
function _Goblin_action_commit_preview() {
    local additional_context="$1"
    local commit_message
    # Generate AI commit message
    echo
    ...
    # Build commit command with optional additional context
    if [[ -n "$additional_context" ]]; then
        commit_message=$(FORCE_COLOR=true CLICOLOR_FORCE=1 $_GOBLIN_BIN commit --preview --max-diff "$_GOBLIN_MAX_COMMIT_DIFF" $additional_context)
    else
        commit_message=$(FORCE_COLOR=true CLICOLOR_FORCE=1 $_GOBLIN_BIN commit --preview --max-diff "$_GOBLIN_MAX_COMMIT_DIFF")
    fi
