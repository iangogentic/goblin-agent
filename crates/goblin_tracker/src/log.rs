...
pub fn init_tracing(log_path: PathBuf, tracker: Tracker) -> anyhow::Result<Guard> {
    ...
    // Create a filter that only allows logs from goblin_ modules
    let filter = filter::filter_fn(|metadata| metadata.target().starts_with("goblin_"));
    ...
    tracing_subscriber::registry()
        .with(tracing_subscriber::EnvFilter::try_from_env("GOBLIN_LOG").unwrap_or(level))
        .with(fmt_layer)
        .init();
...
fn prepare_writer(
    ...
    let ((non_blocking, guard), env) = if can_track() {
        ...
        (
            ...
            tracing_subscriber::EnvFilter::new("goblin=info"),
        )
    } else {
        let append = tracing_appender::rolling::daily(log_path, "goblin.log");
        (
            tracing_appender::non_blocking(append),
            tracing_subscriber::EnvFilter::new("goblin=debug"),
        )
    };
