...
use goblin_app::utils::format_display_path;
use goblin_app::{
...
#[derive(Clone)]
pub struct GoblinChatRequest<F> {
    infra: Arc<F>,
}
...
pub mod tests {
    ...
    use base64::Engine;
    use bytes::Bytes;
    use goblin_app::domain::{AttachmentContent, Environment};
    use goblin_app::utils::compute_hash;
    use goblin_app::{
    ...
    use goblin_domain::{ConfigOperation, FileInfo};
    use futures::stream;
    ...
    async fn test_add_url_with_text_file() {
        ...
        let chat_request = GoblinChatRequest::new(infra.clone());
