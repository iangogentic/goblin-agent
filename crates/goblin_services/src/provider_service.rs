...
use anyhow::Result;
use goblin_app::ProviderService;
...
pub struct GoblinProviderService<R> {
    repository: Arc<R>,
}

impl<R> GoblinProviderService<R> {
    /// Creates a new GoblinProviderService instance
    ...
    fn render_url_template(
        &self,
        template: &str,
        params: &HashMap<goblin_domain::URLParam, goblin_domain::URLParamValue>,
    ) -> Result<Url> {
        ...
        let handlebars = goblin_app::TemplateEngine::handlebar_instance();
...
impl<R: ChatRepository + ProviderRepository> ProviderService for GoblinProviderService<R> {
    async fn chat(
        &self,
        model_id: &ModelId,
        context: Context,
...
#[cfg(test)]
mod tests {
    use goblin_app::domain::ProviderId;
    use goblin_domain::{
    ...
    fn test_provider() -> Provider<Url> {
        Provider {
            ...
            response: Some(goblin_app::domain::ProviderResponse::OpenAI),
            url: Url::parse("https://api.openai.com/v1/chat/completions").unwrap(),
            auth_methods: vec![AuthMethod::ApiKey],
            url_params: vec![],
            credential: Some(AuthCredential {
                id: ProviderId::OPENAI,
                auth_details: AuthDetails::ApiKey(goblin_domain::ApiKey::from(
                    "test-key".to_string(),
                )),
    ...
    fn test_template_provider() -> ProviderTemplate {
        Provider {
            ...
            provider_type: ProviderType::Llm,
            response: Some(goblin_app::domain::ProviderResponse::OpenAI),
            url: Template::<goblin_domain::URLParameters>::new(
            ...
            models: Some(ModelSource::Url(
                Template::<goblin_domain::URLParameters>::new("https://api.openai.com/v1/models"),
            )),
            custom_headers: None,
        }
    }
    ...
    async fn test_models_delegates_to_repository() {
        ...
        let service = GoblinProviderService::new(repository);
