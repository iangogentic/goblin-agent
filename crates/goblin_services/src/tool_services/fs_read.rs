...
use goblin_domain::{FileInfo, Image};
...
pub struct GoblinFsRead<F>(Arc<F>);

impl<F> GoblinFsRead<F> {
...
impl<F: FileInfoInfra + EnvironmentInfra + InfraFsReadService> FsReadService for GoblinFsRead<F> {
    async fn read(
        &self,
        path: String,
            ...
            assert_file_size(&*self.0, path, config.max_image_size_bytes)
                ...
                .with_context(|| {
                    if mime_type == "application/pdf" {
                        "PDF exceeds size limit. Use a smaller PDF or increase GOBLIN_MAX_IMAGE_SIZE."
                    } else {
