...
use anyhow::bail;
use goblin_app::domain::Environment;
use goblin_app::{CommandInfra, EnvironmentInfra, ShellOutput, ShellService};
...
/// unrestricted access, advise users to run goblin CLI with '-u' flag. Returns
/// complete output including stdout, stderr, and exit code for diagnostic
/// purposes.
pub struct GoblinShell<I> {
    env: Environment,
    infra: Arc<I>,
}

impl<I: EnvironmentInfra> GoblinShell<I> {
...
impl<I: CommandInfra + EnvironmentInfra> ShellService for GoblinShell<I> {
    async fn execute(
        &self,
        command: String,
        cwd: PathBuf,
...
mod tests {
    ...
    use goblin_app::domain::{CommandOutput, Environment};
    use goblin_app::{CommandInfra, EnvironmentInfra, ShellService};
    use goblin_domain::ConfigOperation;
    ...
    async fn test_shell_service_forwards_env_vars() {
        let fixture = GoblinShell::new(Arc::new(MockCommandInfra {
    ...
    async fn test_shell_service_forwards_no_env_vars() {
        let fixture = GoblinShell::new(Arc::new(MockCommandInfra { expected_env_vars: None }));
