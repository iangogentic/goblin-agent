...
use bytes::Bytes;
use goblin_app::domain::PatchOperation;
use goblin_app::{FileWriterInfra, FsPatchService, PatchOutput, compute_hash};
use goblin_domain::{FuzzySearchRepository, SearchMatch, SnapshotRepository, ValidationRepository};
use thiserror::Error;
use tokio::fs;
...
// Using PatchOperation from goblin_domain

// Using FSPatchInput from goblin_domain
...
pub struct GoblinFsPatch<F> {
    infra: Arc<F>,
}

impl<F> GoblinFsPatch<F> {
...
impl<F: FileWriterInfra + SnapshotRepository + ValidationRepository + FuzzySearchRepository>
    FsPatchService for GoblinFsPatch<F>
{
    async fn patch(
        &self,
        input_path: String,
