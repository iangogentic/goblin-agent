...
use goblin_domain::{FSSearch, OutputMode};
...
pub struct GoblinFsSearch<W> {
    infra: Arc<W>,
}

impl<W> GoblinFsSearch<W> {
...
impl<W: WalkerInfra + FileReaderInfra + FileInfoInfra> FsSearchService for GoblinFsSearch<W> {
...
impl<W: WalkerInfra + FileReaderInfra + FileInfoInfra> GoblinFsSearch<W> {
...
mod test {
    ...
    use goblin_app::{WalkedFile, Walker};
    use goblin_domain::{FSSearch, OutputMode};
    use tokio::fs;
    ...
    async fn test_basic_content_search() {
        ...
        let actual = GoblinFsSearch::new(Arc::new(MockInfra::default()))
            .search(params)
            .await
            .unwrap();
    ...
    async fn test_path_defaults_to_cwd() {
        ...
        let result = GoblinFsSearch::new(Arc::new(MockInfra::default()))
            .search(params)
            .await;
