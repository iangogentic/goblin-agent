...
use goblin_app::SkillFetchService;
use goblin_domain::Skill;
...
pub struct GoblinSkillFetch<R> {
    repository: Arc<R>,
    cache: OnceCell<Vec<Skill>>,
}

impl<R> GoblinSkillFetch<R> {
...
impl<R: goblin_domain::SkillRepository> SkillFetchService for GoblinSkillFetch<R> {
...
impl<R: goblin_domain::SkillRepository> GoblinSkillFetch<R> {
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl goblin_domain::SkillRepository for MockSkillRepository {
    ...
    async fn test_fetch_skill_found() {
        ...
        let fetch_service = GoblinSkillFetch::new(Arc::new(repo));
