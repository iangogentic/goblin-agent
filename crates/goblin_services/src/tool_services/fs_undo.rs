...
use goblin_app::{FileInfoInfra, FileReaderInfra, FsUndoOutput, FsUndoService};
use goblin_domain::SnapshotRepository;
...
#[derive(Default)]
pub struct GoblinFsUndo<F> {
    infra: Arc<F>,
}

impl<F> GoblinFsUndo<F> {
...
impl<F: FileInfoInfra + FileReaderInfra + SnapshotRepository> FsUndoService for GoblinFsUndo<F> {
