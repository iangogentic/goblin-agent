...
use anyhow::Context;
use goblin_app::domain::Image;
use goblin_app::{EnvironmentInfra, FileInfoInfra, ImageReadService};
use strum_macros::{Display, EnumString};

use crate::utils::assert_absolute_path;

pub struct GoblinImageRead<F>(Arc<F>);
...
impl<F> GoblinImageRead<F> {
    pub fn new(infra: Arc<F>) -> Self {
        Self(infra)
    }
}
#[async_trait::async_trait]
impl<F: FileInfoInfra + EnvironmentInfra + goblin_app::FileReaderInfra> ImageReadService
    for GoblinImageRead<F>
{
    async fn read_image(&self, path: String) -> anyhow::Result<Image> {
        ...
        .await
        .with_context(
            || "Image exceeds size limit. Compress the image or increase GOBLIN_MAX_IMAGE_SIZE.",
        )?;
