...
use dashmap::DashMap;
use goblin_app::domain::{AgentId, Error, ModelId, ProviderId};
use goblin_app::{AgentRepository, EnvironmentInfra};
use goblin_domain::Agent;
...
/// access.
pub struct GoblinAgentRegistryService<R> {
...
impl<R> GoblinAgentRegistryService<R> {
...
impl<R: AgentRepository + EnvironmentInfra> GoblinAgentRegistryService<R> {
    ...
    ///
    /// Reads the default provider and model from [`GoblinConfig`] and passes
...
impl<R: AgentRepository + EnvironmentInfra + Send + Sync> goblin_app::AgentRegistry
    for GoblinAgentRegistryService<R>
{
