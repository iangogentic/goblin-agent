...
use anyhow::Result;
use goblin_app::domain::File;
use goblin_app::{
    DirectoryReaderInfra, EnvironmentInfra, FileDiscoveryService, Walker, WalkerInfra,
};

pub struct GoblinDiscoveryService<F> {
    service: Arc<F>,
}

impl<F> GoblinDiscoveryService<F> {
    pub fn new(service: Arc<F>) -> Self {
        Self { service }
    }
}

impl<F: EnvironmentInfra + WalkerInfra> GoblinDiscoveryService<F> {
...
mod tests {
    ...
    use goblin_app::{EnvironmentInfra, WalkedFile};
    ...
    async fn test_list_current_directory_sorts_dirs_first() {
        ...
        let service = GoblinDiscoveryService::new(Arc::new(fixture));
