...
use anyhow::Result;
use goblin_app::ConversationService;
use goblin_app::domain::{Conversation, ConversationId};
use goblin_domain::ConversationRepository;

/// Service for managing conversations, including creation, retrieval, and
/// updates
#[derive(Clone)]
pub struct GoblinConversationService<S> {
    conversation_repository: Arc<S>,
}

impl<S: ConversationRepository> GoblinConversationService<S> {
    /// Creates a new GoblinConversationService with the provided repository
...
impl<S: ConversationRepository> ConversationService for GoblinConversationService<S> {
        ...
        T: Send,
    {
        let mut conversation = self
            ...
            .get_conversation(id)
            .await?
            .ok_or_else(|| goblin_app::domain::Error::ConversationNotFound(*id))?;
