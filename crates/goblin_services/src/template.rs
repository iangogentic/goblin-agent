...
use anyhow::Context;
use goblin_app::{EnvironmentInfra, FileReaderInfra, TemplateService};
...
#[derive(Clone)]
pub struct GoblinTemplateService<F> {
    hb: Arc<OnceCell<RwLock<Handlebars<'static>>>>,
    infra: Arc<F>,
}

impl<F: EnvironmentInfra + FileReaderInfra> GoblinTemplateService<F> {
    ...
    async fn get_hb(&self) -> &RwLock<Handlebars<'static>> {
        self.hb
            .get_or_init(|| async { RwLock::new(goblin_app::TemplateEngine::handlebar_instance()) })
            .await
    }
...
impl<F: EnvironmentInfra + FileReaderInfra> TemplateService for GoblinTemplateService<F> {
...
mod tests {
    ...
    async fn test_render_simple_template() {
        ...
        let service = GoblinTemplateService::new(Arc::new(MockCompositeService::new()));
        let data = json!({
            "name": "Goblin",
            "version": "1.0",
        ...
        let expected = "App: Goblin v1.0 - Features: templates, rendering, handlebars";
    ...
    async fn test_render_partial_system_info() {
        ...
        let actual = service
            .render_template(Template::new("{{> goblin-partial-system-info.md }}"), &data)
            .await
            .unwrap();
