...
use anyhow::Result;
use goblin_app::DirectoryReaderInfra;
use goblin_fs::GoblinFS;
use goblin_walker::Walker;
use futures::StreamExt;
use glob::Pattern;

/// Service for reading multiple files from a directory asynchronously
pub struct GoblinDirectoryReaderService {
    parallel_file_reads: usize,
}

impl GoblinDirectoryReaderService {
    ...
    async fn list_directory_entries(&self, directory: &Path) -> Result<Vec<(PathBuf, bool)>> {
        ...
        if !GoblinFS::exists(directory) || GoblinFS::is_file(directory) {
            return Ok(vec![]);
        }
    ...
    async fn read_directory_files(
        ...
        let mut dir = GoblinFS::read_dir(directory).await?;
        ...
        let files = futures::stream::iter(file_paths)
            .map(|path| async move {
                ...
                match GoblinFS::read_to_string(&path).await {
...
impl DirectoryReaderInfra for GoblinDirectoryReaderService {
    async fn list_directory_entries(&self, directory: &Path) -> Result<Vec<(PathBuf, bool)>> {
...
mod tests {
    ...
    async fn test_read_directory_files_with_filter() {
        ...
        let actual = GoblinDirectoryReaderService::new(64)
    ...
    async fn test_read_directory_files_without_filter() {
        ...
        let mut actual = GoblinDirectoryReaderService::new(64)
