...
use goblin_app::{AuthStrategy, OAuthHttpProvider, StrategyFactory};
use goblin_domain::{
...
async fn poll_for_tokens(
    device_code: &goblin_domain::DeviceCode,
    config: &OAuthConfig,
    timeout: Duration,
...
pub struct GoblinAuthStrategyFactory {}

impl Default for GoblinAuthStrategyFactory {
    fn default() -> Self {
        Self::new()
    }
}

impl GoblinAuthStrategyFactory {
    pub fn new() -> Self {
        Self {}
    }
}

impl StrategyFactory for GoblinAuthStrategyFactory {
    ...
    fn create_auth_strategy(
        ...
        match auth_method {
            goblin_domain::AuthMethod::ApiKey => Ok(AnyAuthStrategy::ApiKey(ApiKeyStrategy::new(
                provider_id,
                required_params,
            ))),
            goblin_domain::AuthMethod::OAuthCode(config) => {
            ...
            goblin_domain::AuthMethod::OAuthDevice(config) => {
            ...
            goblin_domain::AuthMethod::GoogleAdc => Ok(AnyAuthStrategy::GoogleAdc(
                GoogleAdcStrategy::new(provider_id, required_params),
            )),
            goblin_domain::AuthMethod::CodexDevice(config) => Ok(AnyAuthStrategy::CodexDevice(
...
mod tests {
    ...
    fn test_create_auth_strategy_api_key() {
        let factory = GoblinAuthStrategyFactory::new();
    ...
    fn test_create_auth_strategy_codex_device() {
        ...
        let actual = factory.create_auth_strategy(
            ProviderId::CODEX,
            goblin_domain::AuthMethod::CodexDevice(config),
            vec![],
        );
        assert!(actual.is_ok());
