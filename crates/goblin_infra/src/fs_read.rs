...
use anyhow::Result;
use goblin_app::FileReaderInfra;
use futures::{StreamExt, stream};

#[derive(Clone)]
pub struct GoblinFileReadService;

impl Default for GoblinFileReadService {
    fn default() -> Self {
        Self
    }
}

impl GoblinFileReadService {
    pub fn new() -> Self {
        Self
    }
}
...
impl FileReaderInfra for GoblinFileReadService {
    async fn read_utf8(&self, path: &Path) -> Result<String> {
        goblin_fs::GoblinFS::read_utf8(path).await
    }
    ...
    async fn read(&self, path: &Path) -> Result<Vec<u8>> {
        goblin_fs::GoblinFS::read(path).await
    }
    ...
    async fn range_read_utf8(
        ...
        start_line: u64,
        end_line: u64,
    ) -> Result<(String, goblin_domain::FileInfo)> {
        goblin_fs::GoblinFS::read_range_utf8(path, start_line, end_line).await
    }
}
...
mod tests {
    ...
    async fn test_read_batch_utf8() {
        let fixture = GoblinFileReadService::new();
