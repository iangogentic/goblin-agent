...
pub struct GoblinWalkerService;

impl GoblinWalkerService {
    pub fn new() -> Self {
        Self
    }

    pub async fn walk(&self, config: Walker) -> Result<Vec<WalkedFile>> {
        // Convert domain config to goblin_walker config
        ...
        {
            goblin_walker::Walker::max_all()
        } else {
            goblin_walker::Walker::min_all()
        };
...
mod tests {
    ...
    async fn test_walker_service_basic_functionality() {
        ...
        let service = GoblinWalkerService::new();
