...
use goblin_config::{GoblinConfig, TlsBackend, TlsVersion};
...
impl<F: goblin_app::FileWriterInfra + 'static> GoblinHttpInfra<F> {
    /// Creates a new [`GoblinHttpInfra`] from a resolved [`GoblinConfig`].
    pub fn new(config: GoblinConfig, file_writer: Arc<F>) -> Self {
        let http = config.http.unwrap_or(goblin_config::HttpConfig {
    ...
    fn headers(&self, headers: Option<HeaderMap>) -> HeaderMap {
        ...
        if !headers.contains_key("User-Agent") {
            headers.insert("User-Agent", HeaderValue::from_static("Goblin"));
        }
        headers.insert("X-Title", HeaderValue::from_static("goblin"));
        headers.insert(
            "x-app-version",
        ...
        headers.insert(
            "HTTP-Referer",
            HeaderValue::from_static("https://goblincode.dev"),
        );
        headers.insert(
...
impl<F: goblin_app::FileWriterInfra + 'static> HttpInfra for GoblinHttpInfra<F> {
...
mod tests {
    ...
    fn create_test_config(debug_requests: Option<PathBuf>) -> GoblinConfig {
        GoblinConfig { debug_requests, ..Default::default() }
    }
    ...
    async fn test_debug_requests_none_does_not_write() {
        ...
        let http = GoblinHttpInfra::new(config, Arc::new(file_writer.clone()));
    ...
    async fn test_debug_requests_with_valid_path() {
        let file_writer = MockFileWriter::new();
        let debug_path = PathBuf::from("/tmp/goblin-test/debug.json");
    ...
    async fn test_debug_requests_post_writes_body() {
        let file_writer = MockFileWriter::new();
        let debug_path = PathBuf::from("/tmp/goblin-test/debug-post.json");
        let config = create_test_config(Some(debug_path.clone()));
        let http = GoblinHttpInfra::new(config, Arc::new(file_writer.clone()));
