...
use goblin_app::EnvironmentInfra;
use goblin_config::{ConfigReader, GoblinConfig, ModelConfig};
use goblin_domain::{ConfigOperation, Environment};
use tracing::{debug, error};

/// Builds a [`goblin_domain::Environment`] from runtime context only.
///
/// Only the six fields that cannot be sourced from [`GoblinConfig`] are set
...
fn to_environment(cwd: PathBuf) -> Environment {
    Environment {
        ...
        base_path: dirs::home_dir()
            .map(|h| h.join("goblin"))
            .unwrap_or_else(|| PathBuf::from(".").join("goblin")),
    }
}

/// Applies a single [`ConfigOperation`] directly to a [`GoblinConfig`].
///
/// Used by [`GoblinEnvironmentInfra::update_environment`] to mutate the
/// persisted config without an intermediate `Environment` round-trip.
fn apply_config_op(fc: &mut GoblinConfig, op: ConfigOperation) {
    match op {
...
/// caching support.
///
/// Uses [`GoblinConfig::read`] and [`GoblinConfig::write`] for all file I/O and
/// maintains an in-memory cache to reduce disk access. Also handles
/// environment variable discovery via `.env` files and OS APIs.
pub struct GoblinEnvironmentInfra {
    cwd: PathBuf,
    cache: Arc<std::sync::Mutex<Option<GoblinConfig>>>,
}

impl GoblinEnvironmentInfra {
    /// Creates a new [`GoblinEnvironmentInfra`].
    ///
    /// # Arguments
    ...
    /// Reads [`GoblinConfig`] from disk via [`GoblinConfig::read`].
    fn read_from_disk() -> GoblinConfig {
        match GoblinConfig::read() {
            Ok(config) => {
                debug!(config = ?config, "read .goblin.toml");
                config
            }
            Err(e) => {
    ...
    /// Returns the cached [`GoblinConfig`], reading from disk if the cache is
    /// empty.
    fn cached_config(&self) -> GoblinConfig {
...
impl EnvironmentInfra for GoblinEnvironmentInfra {
    ...
    async fn update_environment(&self, ops: Vec<ConfigOperation>) -> anyhow::Result<()> {
        ...
        fc.write()?;
        debug!(config = ?fc, "written .goblin.toml");
...
mod tests {
    ...
    fn test_apply_config_op_set_provider() {
        ...
        let mut fixture = GoblinConfig::default();
        apply_config_op(
            &mut fixture,
