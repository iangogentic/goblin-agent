...
impl crate::GoblinFS {
    ...
    pub async fn read_range_utf8<T: AsRef<Path>>(
        ...
        let content_hash = crate::GoblinFS::compute_hash(content.as_ref());
...
mod test {
    ...
    async fn test_read_range_utf8() -> Result<()> {
        ...
        let full_hash = crate::GoblinFS::compute_hash(content);

        // Test reading a range of lines
        let (result, info) = crate::GoblinFS::read_range_utf8(file.path(), 2, 5).await?;
        ...
        let (result, info) = crate::GoblinFS::read_range_utf8(file.path(), 1, 3).await?;
        ...
        // Test reading to end
        let (result, info) = crate::GoblinFS::read_range_utf8(file.path(), 8, 10).await?;
        ...
        let (result, info) = crate::GoblinFS::read_range_utf8(file.path(), 1, 10).await?;
        ...
        // Test single line
        let (result, info) = crate::GoblinFS::read_range_utf8(file.path(), 5, 5).await?;
        ...
        let (result, info) = crate::GoblinFS::read_range_utf8(file.path(), 1, 1).await?;
        assert_eq!(result, "Line 1");
        assert_eq!(info.start_line, 1);
        ...
        // Test invalid ranges
        assert!(
            crate::GoblinFS::read_range_utf8(file.path(), 8, 5)
                .await
                .is_err()
        );
        assert!(
            crate::GoblinFS::read_range_utf8(file.path(), 15, 10)
                .await
                .is_err()
        );
        assert!(
            crate::GoblinFS::read_range_utf8(file.path(), 0, 5)
                .await
                .is_err()
        );
    ...
    async fn test_utf8_multi_line_handling() -> Result<()> {
        ...
        let (result, info) = crate::GoblinFS::read_range_utf8(file.path(), 2, 3).await?;
    ...
    async fn test_invalid_utf8_handling() -> Result<()> {
        ...
        let result = crate::GoblinFS::read_range_utf8(&file.path(), 1, 4).await;
        assert!(result.is_ok());
        Ok(())
    }
    ...
    async fn test_end_line_capping() -> Result<()> {
        ...
        let (actual_content, actual_info) =
            crate::GoblinFS::read_range_utf8(file.path(), 1, 5).await?;
        ...
        let (actual_content, actual_info) =
            crate::GoblinFS::read_range_utf8(file.path(), 1, 6).await?;
        assert_eq!(actual_content, content);
        assert_eq!(
            (
                actual_info.start_line,
        ...
        let (actual_content, actual_info) =
            crate::GoblinFS::read_range_utf8(file.path(), 1, 10000).await?;
        assert_eq!(actual_content, content);
        assert_eq!(
            (
                actual_info.start_line,
        ...
        let (actual_content, actual_info) =
            crate::GoblinFS::read_range_utf8(file.path(), 3, 100).await?;
        ...
        let (actual_content, actual_info) =
            crate::GoblinFS::read_range_utf8(file.path(), 5, 100).await?;
    ...
    async fn test_large_file_ranges() -> Result<()> {
        ...
        let (actual_content, actual_info) =
            crate::GoblinFS::read_range_utf8(file.path(), 1, 50).await?;
        ...
        let (actual_content, actual_info) =
            crate::GoblinFS::read_range_utf8(file.path(), 2500, 3500).await?;
        ...
        let (actual_content, actual_info) =
            crate::GoblinFS::read_range_utf8(file.path(), 4990, 6000).await?;
