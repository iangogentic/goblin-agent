...
1. **Entry Point**: `crates/goblin_app/src/tool_executor.rs:336`

   ```rust
   if env.enable_permissions && self.check_tool_permission(&tool_input, context).await?
   ```

2. **Permission Check Method**: `crates/goblin_app/src/tool_executor.rs:48-72`
   - The `check_tool_permission()` method is called before executing any tool
   - It converts the tool catalog to a policy operation

3. **Policy Operation Conversion**: `crates/goblin_domain/src/tools/catalog.rs:680-684`
...
4. **Permission Decision**: `crates/goblin_services/src/policy.rs:163-208`
...
5. **Rule Matching**: `crates/goblin_domain/src/policies/rule.rs:88-96`
...
6. **Actual Fetch Execution**: `crates/goblin_app/src/tool_executor.rs:282-284`
