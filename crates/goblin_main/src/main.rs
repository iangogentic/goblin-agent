...
use anyhow::Result;
use clap::Parser;
use goblin_api::GoblinAPI;
use goblin_domain::TitleFormat;
use goblin_main::{Cli, Sandbox, TitleDisplayExt, UI, tracker};
...
async fn main() -> Result<()> {
    ...
    let mut ui = UI::init(cli, move || GoblinAPI::init(cwd.clone()))?;
    ui.run().await;
...
#[cfg(test)]
mod tests {
    use goblin_main::TopLevelCommand;
    ...
    fn test_stdin_detection_logic() {
        ...
        let cli_with_prompt = Cli::parse_from(["goblin", "--prompt", "existing prompt"]);
        ...
        let cli_no_prompt = Cli::parse_from(["goblin"]);
    ...
    fn test_cli_parsing_with_short_flag() {
        ...
        let cli_with_short_prompt = Cli::parse_from(["goblin", "-p", "short flag prompt"]);
        assert_eq!(
    ...
    fn test_cli_parsing_other_flags_work_with_piping() {
        ...
        let cli_with_flags = Cli::parse_from(["goblin", "--verbose"]);
    ...
    fn test_commit_command_diff_field_initially_none() {
        ...
        let cli = Cli::parse_from(["goblin", "commit", "--preview"]);
