...
use goblin_api::{Conversation, Environment, GoblinConfig, Metrics, Role, Usage, UserUsage};
use goblin_tracker::VERSION;
use num_format::{Locale, ToFormattedString};

use crate::display_constants::markers;
use crate::model::GoblinCommandManager;
...
impl From<&GoblinConfig> for Info {
    fn from(config: &GoblinConfig) -> Self {
        let mut info = Info::new();
        ...
        info = info
            ...
            .add_key_value("GoblinCode Service URL", config.services_url.to_string())
...
/// Create an info instance for available commands from a GoblinCommandManager
impl From<&GoblinCommandManager> for Info {
    fn from(command_manager: &GoblinCommandManager) -> Self {
        ...
        info = info
            ...
            .add_key_value("<CTRL+D>", "Quit Goblin interactive shell")
    ...
    fn from(user_usage: &UserUsage) -> Self {
        ...
        if plan.is_upgradeable() {
            info = info.add_key_value(
                "Subscription",
                format!(
                    "{} [Upgrade https://app.goblincode.dev/app/billing]",
...
fn format_user_message(msg: &goblin_api::ContextMessage) -> Option<String> {
    let content = msg
        .as_value()
...
mod tests {
    ...
    use goblin_api::{Environment, EventValue};
    ...
    fn test_metrics_info_display() {
        use goblin_api::Metrics;
        use goblin_domain::{FileOperation, ToolKind};
    ...
    fn test_conversation_info_display() {
        ...
        let fixture = Conversation {
            ...
            context: None,
            metrics,
            metadata: goblin_domain::MetaData::new(Utc::now()),
        };
    ...
    fn test_conversation_info_display_with_task() {
        use chrono::Utc;
        use goblin_api::{Context, ContextMessage, ConversationId, Role};
        ...
        let context = Context::default()
            ...
            .add_message(ContextMessage::Text(
                goblin_domain::TextMessage::new(Role::User, "First user message")
            ...
            .add_message(ContextMessage::Text(
                goblin_domain::TextMessage::new(Role::User, "Create a new feature")
    ...
    fn test_info_from_command_manager() {
        let command_manager = super::GoblinCommandManager::default();
