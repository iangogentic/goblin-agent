...
async fn execute_update_command(api: Arc<impl API>, auto_update: bool) {
    ...
    let output = api
        .execute_shell_command_raw("curl -fsSL https://goblincode.dev/cli | sh")
        .await;
        ...
        Ok(output) => {
            if output.success() {
                let should_exit = if auto_update {
                    ...
                    let answer = goblin_select::GoblinWidget::confirm(
                        "You need to close goblin to complete update. Do you want to close it now?",
                    )
                    .with_default(true)
                    .prompt();
...
async fn confirm_update(version: Version) -> bool {
    let answer = GoblinWidget::confirm(format!(
...
pub async fn on_update(api: Arc<impl API>, update: Option<&Update>) {
    ...
    let informer = update_informer::new(registry::GitHub, "antinomyhq/goblin", VERSION)
