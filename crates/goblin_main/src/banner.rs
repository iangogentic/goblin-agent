...
///
/// * `GOBLIN_BANNER` - Optional custom banner text to display instead of the
///   default
pub fn display(cli_mode: bool) -> io::Result<()> {
    // Check for custom banner via environment variable
    let mut banner = std::env::var("GOBLIN_BANNER")
        .ok()
        .filter(|s| !s.is_empty())
...
fn display_zsh_encouragement() {
    let tip = DisplayBox::new(vec![
        ...
        format!(
            "{} {} {}",
            "·".dimmed(),
            "Set up goblin via our zsh plugin:".dimmed(),
            "goblin zsh setup".bold().green(),
        ),
        format!(
            "{} {} {}",
            "·".dimmed(),
            "Learn more:".dimmed(),
            "https://goblincode.dev/docs/zsh-support".cyan()
        ),
    ]);
    println!("{}", tip);
}
