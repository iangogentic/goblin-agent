...
pub fn generate_zsh_plugin() -> Result<String> {
    ...
    for file in goblin_embed::files(&ZSH_PLUGIN_LIB) {
    ...
    generate(Zsh, &mut cmd, "goblin", &mut completions);
    ...
    output.push_str("\n_GOBLIN_PLUGIN_LOADED=$(date +%s)\n");

    Ok(output)
}

/// Generates the ZSH theme for Goblin
pub fn generate_zsh_theme() -> Result<String> {
    let mut content =
        super::normalize_script(include_str!("../../../../shell-plugin/goblin.theme.zsh"));

    // Set environment variable to indicate theme is loaded (with timestamp)
    content.push_str("\n_GOBLIN_THEME_LOADED=$(date +%s)\n");
...
fn create_temp_zsh_script(script_content: &str) -> Result<(tempfile::TempDir, PathBuf)> {
    ...
    let script_path = temp_dir.path().join("goblin_script.zsh");
...
/// * `goblin_editor` - If Some(editor), adds GOBLIN_EDITOR export to .zshrc
///
/// # Errors
///
/// Returns error if:
...
/// - Invalid goblin markers are found (incomplete or incorrectly ordered)
/// - A backup of the existing .zshrc cannot be created
pub fn setup_zsh_integration(
    disable_nerd_font: bool,
    goblin_editor: Option<&str>,
) -> Result<ZshSetupResult> {
    const START_MARKER: &str = "# >>> goblin initialize >>>";
    const END_MARKER: &str = "# <<< goblin initialize <<<";
    const GOBLIN_INIT_CONFIG_RAW: &str = include_str!("../../../../shell-plugin/goblin.setup.zsh");
    let goblin_init_config = super::normalize_script(GOBLIN_INIT_CONFIG_RAW);
    ...
    // Build the goblin config block with markers
    let mut goblin_config: Vec<String> = vec![START_MARKER.to_string()];
    goblin_config.extend(goblin_init_config.lines().map(String::from));

    // Add nerd font configuration if requested
    if disable_nerd_font {
        goblin_config.push(String::new()); // Add blank line before comment
        goblin_config.push(
            "# Disable Nerd Fonts (set during setup - icons not displaying correctly)".to_string(),
        );
        goblin_config.push("# To re-enable: remove this line and install a Nerd Font from https://www.nerdfonts.com/".to_string());
        goblin_config.push("export NERD_FONT=0".to_string());
    }

    // Add editor configuration if requested
    if let Some(editor) = goblin_editor {
        goblin_config.push(String::new()); // Add blank line before comment
        goblin_config.push("# Editor for editing prompts (set during setup)".to_string());
        goblin_config.push("# To change: update GOBLIN_EDITOR or remove to use $EDITOR".to_string());
        goblin_config.push(format!("export GOBLIN_EDITOR=\"{}\"", editor));
    }

    goblin_config.push(END_MARKER.to_string());

    // Add or update goblin configuration block based on marker state
    let (new_content, config_action) = match marker_state {
        MarkerState::Valid { start, end } => {
            // Markers exist - replace content between them
            lines.splice(start..=end, goblin_config.iter().cloned());
            (lines.join("\n") + "\n", "updated")
        }
        MarkerState::Invalid { start, end } => {
            ...
            let mut error =
                anyhow::anyhow!("Invalid goblin markers found in {}", zshrc_path.display());
        ...
        MarkerState::NotFound => {
            ...
            lines.extend(goblin_config.iter().cloned());
            (lines.join("\n") + "\n", "added")
        }
    };
    ...
    Ok(ZshSetupResult {
        message: format!("goblin plugins {}", config_action),
        backup_path,
    })
}
...
mod tests {
    ...
    fn test_run_zsh_doctor_streaming() {
        ...
        unsafe {
            std::env::set_var("GOBLIN_SKIP_INTERACTIVE", "1");
        }
        ...
        unsafe {
            std::env::remove_var("GOBLIN_SKIP_INTERACTIVE");
        }
    ...
    fn test_setup_zsh_integration_without_nerd_font_config() {
        ...
        assert!(content.contains("# >>> goblin initialize >>>"));
        assert!(content.contains("# <<< goblin initialize <<<"));
    }
    ...
    fn test_setup_zsh_integration_with_editor() {
        ...
        // Should contain GOBLIN_EDITOR with explanatory comments
        assert!(
            content.contains("export GOBLIN_EDITOR=\"code --wait\""),
            "Content should contain GOBLIN_EDITOR:\n{}",
            content
        );
        assert!(
        ...
        assert!(
            content.contains("# To change: update GOBLIN_EDITOR or remove to use $EDITOR"),
    ...
    fn test_setup_zsh_integration_with_both_configs() {
        ...
        assert!(
            content.contains("export GOBLIN_EDITOR=\"vim\""),
            "Content should contain GOBLIN_EDITOR:\n{}",
            content
        );
    ...
    fn test_setup_zsh_integration_updates_existing_markers() {
        ...
        assert!(
            !content.contains("export GOBLIN_EDITOR"),
            "Should not contain GOBLIN_EDITOR after first setup"
        );
        ...
        assert!(
            content.contains("export GOBLIN_EDITOR=\"nvim\""),
            "Should contain GOBLIN_EDITOR after update:\n{}",
            content
        );
        ...
        assert_eq!(
            content.matches("# >>> goblin initialize >>>").count(),
            1,
