...
/// Writes a [`GoblinConfig`] to the user configuration file on disk.
...
impl ConfigWriter {
    ...
    pub fn new(config: GoblinConfig) -> Self {
        Self { config }
    }
    ...
    ///
    /// The output includes a leading `$schema` key pointing to the Goblin
    ...
    pub fn write(&self, path: &Path) -> crate::Result<()> {
        ...
        let contents =
            format!("\"$schema\" = \"https://goblincode.dev/schema.json\"\n\n{config_toml}");
