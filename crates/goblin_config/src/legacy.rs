...
use crate::{GoblinConfig, ModelConfig};

/// Intermediate representation of the legacy `~/goblin/.config.json` format.
///
...
impl LegacyConfig {
    /// Reads the legacy `~/goblin/.config.json` file at `path`, parses it, and
    /// returns the equivalent TOML representation as a [`String`].
    ///
    /// Because every field in [`GoblinConfig`] is `Option`, fields not covered
    ...
    pub(crate) fn read(path: &PathBuf) -> crate::Result<String> {
        ...
        let goblin_config = config.into_goblin_config();
        let content = toml_edit::ser::to_string_pretty(&goblin_config)?;
        Ok(content)
    }

    /// Converts a [`LegacyConfig`] into the fields of [`GoblinConfig`] that it
    /// covers, leaving all other fields at their defaults (`None`).
    fn into_goblin_config(self) -> GoblinConfig {
        ...
        GoblinConfig { session, commit, suggest, ..Default::default() }
    }
}
