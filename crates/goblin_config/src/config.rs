...
/// Top-level Goblin configuration merged from all sources (defaults, file,
/// environment).
...
pub struct GoblinConfig {
    ...
    /// Base URL of the Goblin services API used for semantic search and
    /// indexing.
    #[serde(default)]
    #[dummy(expr = "\"https://api.goblincode.dev/api\".to_string()")]
    ...
    // --- Workflow fields ---
    /// Configuration for automatic Goblin updates.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl GoblinConfig {
    ...
    pub fn read() -> crate::Result<GoblinConfig> {
...
mod tests {
    ...
    fn test_f32_temperature_round_trip() {
        let fixture = GoblinConfig { temperature: Some(Decimal(0.1)), ..Default::default() };
    ...
    fn test_f32_top_p_round_trip() {
        let fixture = GoblinConfig { top_p: Some(Decimal(0.9)), ..Default::default() };
