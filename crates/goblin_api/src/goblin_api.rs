...
pub struct GoblinAPI<S, F> {
    services: Arc<S>,
    infra: Arc<F>,
}

impl<A, F> GoblinAPI<A, F> {
    pub fn new(services: Arc<A>, infra: Arc<F>) -> Self {
        Self { services, infra }
    }

    /// Creates a GoblinApp instance with the current services
    fn app(&self) -> GoblinApp<A>
    where
        A: Services,
    {
        GoblinApp::new(self.services.clone())
    }
}

impl GoblinAPI<GoblinServices<GoblinRepo<GoblinInfra>>, GoblinRepo<GoblinInfra>> {
    pub fn init(cwd: PathBuf) -> Self {
        let infra = Arc::new(GoblinInfra::new(cwd));
        let repo = Arc::new(GoblinRepo::new(infra.clone()));
        let app = Arc::new(GoblinServices::new(repo.clone()));
        GoblinAPI::new(app, repo)
    }
    ...
    for GoblinAPI<A, F>
{
    ...
    ) -> Result<goblin_app::CommitResult> {
    ...
    async fn user_usage(&self) -> Result<Option<UserUsage>> {
        ...
        if let Some(api_key) = provider
            ...
            .and_then(|c| match &c.auth_details {
                goblin_domain::AuthDetails::ApiKey(key) => Some(key.as_str()),
                _ => None,
            })
        {
    ...
    async fn generate_command(&self, prompt: UserPrompt) -> Result<String> {
        use goblin_app::CommandGenerator;
    ...
    async fn sync_workspace(
        &self,
        path: PathBuf,
    ) -> Result<MpscStream<Result<goblin_domain::SyncProgress>>> {
    ...
    async fn delete_workspaces(&self, workspace_ids: Vec<goblin_domain::WorkspaceId>) -> Result<()> {
    ...
    async fn create_auth_credentials(&self) -> Result<goblin_domain::WorkspaceAuth> {
        self.services.init_auth_credentials().await
    }

    async fn init_workspace(&self, path: PathBuf) -> Result<goblin_domain::WorkspaceId> {
        self.services.init_workspace(path).await
    }

    async fn migrate_env_credentials(&self) -> Result<Option<goblin_domain::MigrationResult>> {
...
impl<A: Send + Sync, F: ConsoleWriter> ConsoleWriter for GoblinAPI<A, F> {
