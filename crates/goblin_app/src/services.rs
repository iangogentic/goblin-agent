...
pub struct TodoWriteOutput {
    ...
    pub todos: Vec<goblin_domain::Todo>,
}
...
pub trait ProviderService: Send + Sync {
    ...
    async fn get_provider(&self, id: goblin_domain::ProviderId) -> anyhow::Result<Provider<Url>>;
    async fn get_all_providers(&self) -> anyhow::Result<Vec<AnyProvider>>;
    async fn upsert_credential(
        &self,
        credential: goblin_domain::AuthCredential,
    ) -> anyhow::Result<()>;
    async fn remove_credential(&self, id: &goblin_domain::ProviderId) -> anyhow::Result<()>;
...
pub trait AppConfigService: Send + Sync {
    ...
    async fn get_provider_model(
        &self,
        provider_id: Option<&goblin_domain::ProviderId>,
    ...
    /// generation).
    async fn set_commit_config(&self, config: goblin_domain::CommitConfig) -> anyhow::Result<()>;
    ...
    /// suggestion generation).
    async fn set_suggest_config(&self, config: goblin_domain::SuggestConfig) -> anyhow::Result<()>;
}
...
pub trait WorkspaceService: Send + Sync {
    ...
    async fn sync_workspace(
        &self,
        path: PathBuf,
    ) -> anyhow::Result<goblin_stream::MpscStream<anyhow::Result<SyncProgress>>>;
...
pub trait ImageReadService: Send + Sync {
    ...
    async fn read_image(&self, path: String) -> anyhow::Result<goblin_domain::Image>;
}
...
pub trait FsSearchService: Send + Sync {
    ...
    /// * `Err(_)` - Search error
    async fn search(&self, params: goblin_domain::FSSearch) -> anyhow::Result<Option<SearchResult>>;
}
...
pub trait AgentRegistry: Send + Sync {
    ...
    async fn get_agents(&self) -> anyhow::Result<Vec<goblin_domain::Agent>>;

    /// Get agent by ID (from registry store)
    async fn get_agent(&self, agent_id: &AgentId) -> anyhow::Result<Option<goblin_domain::Agent>>;
...
pub trait CommandLoaderService: Send + Sync {
    /// Load all command definitions from the goblin/commands directory
    async fn get_commands(&self) -> anyhow::Result<Vec<goblin_domain::Command>>;
}
...
pub trait PolicyService: Send + Sync {
    ...
    async fn check_operation_permission(
        &self,
        operation: &goblin_domain::PermissionOperation,
...
pub trait SkillFetchService: Send + Sync {
    ...
    async fn fetch_skill(&self, skill_name: String) -> anyhow::Result<goblin_domain::Skill>;
    ...
    async fn list_skills(&self) -> anyhow::Result<Vec<goblin_domain::Skill>>;
}
