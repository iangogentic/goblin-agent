...
/// from [`GoblinConfig`] and applied to domain agents at runtime.
pub trait AgentExt {
    ...
    ///
    /// # Arguments
    /// * `config` - The top-level Goblin configuration.
    fn apply_config(self, config: &GoblinConfig) -> Agent;
}
...
impl AgentExt for Agent {
    fn apply_config(self, config: &GoblinConfig) -> Agent {
        ...
        if let Some(ref workflow_compact) = config.compact {
            // Convert goblin_config::Compact to goblin_domain::Compact, then merge.
