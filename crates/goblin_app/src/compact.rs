...
impl Compactor {
    ...
    fn compress_single_sequence(
        ...
        let summary = TemplateEngine::default().render(
            "goblin-partial-summary-frame.md",
        ...
        if let Some(reasoning) = reasoning_details
            && let Some(ContextMessage::Text(msg)) = context
                .messages
                .iter_mut()
                .find(|msg| msg.has_role(goblin_domain::Role::Assistant))
                .map(|msg| &mut **msg)
            && msg
                .reasoning_details
                .as_ref()
...
mod tests {
    ...
    use goblin_domain::MessageEntry;
    ...
    fn render_template(data: &serde_json::Value) -> String {
        TemplateEngine::default()
            .render("goblin-partial-summary-frame.md", data)
            .unwrap()
    }

    #[test]
    fn test_template_engine_renders_summary_frame() {
        use goblin_domain::{ContextSummary, Role, SummaryBlock, SummaryMessage, SummaryToolCall};
        ...
        let messages = vec![
            ...
            SummaryBlock::new(
                Role::Assistant,
                vec![
                    SummaryToolCall::codebase_search(vec![goblin_domain::SearchQuery::new(
    ...
    fn test_template_engine_renders_todo_write() {
        ...
        let messages = vec![
            ...
            SummaryBlock::new(
                ...
                vec![
                    SummaryToolCall {
                        id: Some(goblin_domain::ToolCallId::new("call_1")),
    ...
    async fn test_render_summary_frame_snapshot() {
        ...
        let fixture_json = goblin_test_kit::fixture!("/src/fixtures/conversation.json").await;

        let conversation: goblin_domain::Conversation =
        ...
        let environment = test_environment().cwd(PathBuf::from(
            "/Users/tushar/Documents/Projects/code-goblin-workspace/code-goblin",
        ));
        let compactor = Compactor::new(Compact::new(), environment);
    ...
    fn test_compaction_preserves_usage_information() {
        use goblin_domain::{TokenCount, Usage};
    ...
    fn ctx(pattern: &str) -> Context {
        goblin_domain::MessagePattern::new(pattern).build()
    }
