    ...
    S: EnvironmentInfra + FileDiscoveryService + ProviderService + AppConfigService,
{
    ...
    pub async fn generate(&self, prompt: UserPrompt) -> Result<String> {
        ...
        let rendered_system_prompt = TemplateEngine::default().render(
            "goblin-command-generator-prompt.md",
...
mod tests {
    ...
    impl ProviderService for MockServices {
        ...
        async fn models(&self, _provider: Provider<Url>) -> Result<Vec<goblin_domain::Model>> {
            Ok(vec![])
        }
        ...
        async fn get_all_providers(&self) -> Result<Vec<goblin_domain::AnyProvider>> {
            Ok(vec![])
        }
    ...
    impl AppConfigService for MockServices {
        ...
        async fn get_commit_config(&self) -> Result<Option<goblin_domain::CommitConfig>> {
            Ok(None)
        }

        async fn set_commit_config(&self, _config: goblin_domain::CommitConfig) -> Result<()> {
            Ok(())
        }

        async fn get_suggest_config(&self) -> Result<Option<goblin_domain::SuggestConfig>> {
            Ok(None)
        }

        async fn set_suggest_config(&self, _config: goblin_domain::SuggestConfig) -> Result<()> {
            Ok(())
        }
    }
