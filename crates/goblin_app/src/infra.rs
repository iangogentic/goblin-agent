...
pub trait EnvironmentInfra: Send + Sync {
    ...
    /// Returns the full [`GoblinConfig`] for the current session.
    ///
...
/// This uses the EnvironmentService trait from goblin_domain
...
pub trait AuthStrategy: Send + Sync {
    ...
    async fn init(&self) -> anyhow::Result<goblin_domain::AuthContextRequest>;

    /// Complete authentication flow
    async fn complete(
        &self,
        context_response: goblin_domain::AuthContextResponse,
...
///
/// This trait provides access to fully-resolved domain [`goblin_domain::Agent`]
/// values from:
/// 1. Built-in agents (embedded in the application)
/// 2. Global custom agents (from ~/.goblin/agents/ directory)
/// 3. Project-local agents (from .goblin/agents/ directory in current working
///    directory)
///
/// ## Agent Precedence
