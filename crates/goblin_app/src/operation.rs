...
use goblin_config::GoblinConfig;
use goblin_display::DiffFormat;
use goblin_domain::{
...
pub enum ToolOperation {
    ...
    Skill {
        output: goblin_domain::Skill,
    },
    TodoWrite {
        before: Vec<goblin_domain::Todo>,
        after: Vec<goblin_domain::Todo>,
    },
    TodoRead {
...
fn create_validation_warning(path: &str, errors: &[goblin_domain::SyntaxError]) -> Element {
    Element::new("warning")
...
impl ToolOperation {
    /// Converts this tool operation into a [`goblin_domain::ToolOutput`].
    ///
    /// # Arguments
        ...
        match self {
            ToolOperation::FsRead { input, output } => {
                ...
                if let Some(image) = output.content.as_image() {
                    ...
                    *metrics = metrics.clone().insert(
                    ...
                    return goblin_domain::ToolOutput::image(image.clone());
                }
            ...
            ToolOperation::FsRemove { input, output } => {
                ...
                *metrics = metrics.clone().insert(
                ...
                goblin_domain::ToolOutput::text(elem)
            }
            ...
            ToolOperation::CodebaseSearch { output } => {
                ...
                if output.queries.is_empty() || total_results == 0 {
                    ...
                    for query_result in &output.queries {
                        ...
                        for data in &query_result.results {
                            if let goblin_domain::NodeData::FileChunk(file_chunk) = &data.node {
                ...
                goblin_domain::ToolOutput::text(root)
            }
            ...
            ToolOperation::Shell { output } => {
                ...
                goblin_domain::ToolOutput::text(parent_elem)
            }
            ...
            ToolOperation::TodoWrite { before, after } => {
                ...
                let before_map: std::collections::HashMap<&str, &goblin_domain::Todo> =
...
mod tests {
    ...
    use goblin_domain::{FSRead, FileInfo, ToolValue};
    ...
    fn fixture_config() -> GoblinConfig {
        ...
        GoblinConfig {
            ...
            ..GoblinConfig::default()
        }
    }

    fn to_value(output: goblin_domain::ToolOutput) -> String {
    ...
    fn test_syntax_errors(errors: Vec<(u32, u32, &str)>) -> Vec<goblin_domain::SyntaxError> {
        use goblin_domain::SyntaxError;
    ...
    mod sem_search_helpers {
        use fake::{Fake, Faker};
        use goblin_domain::{CodebaseQueryResult, CodebaseSearchResults, FileChunk, Node, NodeData};
    ...
    fn test_shell_output_no_truncation() {
        let fixture = ToolOperation::Shell {
            output: ShellOutput {
                output: goblin_domain::CommandOutput {
    ...
    fn test_fs_remove_success() {
        let fixture = ToolOperation::FsRemove {
            input: goblin_domain::FSRemove { path: "/home/user/file_to_delete.txt".to_string() },
    ...
    fn test_fs_search_files_with_matches_mode() {
        let fixture = ToolOperation::FsSearch {
            input: goblin_domain::FSSearch {
                ...
                output_mode: Some(goblin_domain::OutputMode::FilesWithMatches),
                ..Default::default()
            },
            output: Some(SearchResult {
                matches: vec![
    ...
    fn test_fs_search_count_mode() {
        let fixture = ToolOperation::FsSearch {
            input: goblin_domain::FSSearch {
                path: Some("/home/user/project".to_string()),
                pattern: "test".to_string(),
                output_mode: Some(goblin_domain::OutputMode::Count),
                ..Default::default()
            },
            output: Some(SearchResult {
                matches: vec![
    ...
    fn test_fs_undo_no_changes() {
        let fixture = ToolOperation::FsUndo {
            input: goblin_domain::FSUndo { path: "/home/user/unchanged_file.txt".to_string() },
    ...
    fn test_fs_undo_file_created() {
        let fixture = ToolOperation::FsUndo {
            input: goblin_domain::FSUndo { path: "/home/user/new_file.txt".to_string() },
            output: FsUndoOutput {
                before_undo: None,
    ...
    fn test_fs_undo_file_removed() {
        let fixture = ToolOperation::FsUndo {
            input: goblin_domain::FSUndo { path: "/home/user/deleted_file.txt".to_string() },
            output: FsUndoOutput {
                before_undo: Some(
    ...
    fn test_fs_undo_file_restored() {
        let fixture = ToolOperation::FsUndo {
            input: goblin_domain::FSUndo { path: "/home/user/restored_file.txt".to_string() },
            output: FsUndoOutput {
    ...
    fn test_fs_undo_success() {
        let fixture = ToolOperation::FsUndo {
            input: goblin_domain::FSUndo { path: "/home/user/test.txt".to_string() },
            output: FsUndoOutput {
    ...
    fn test_net_fetch_truncated() {
        ...
        let truncation_path =
            TempContentFiles::default().stdout(PathBuf::from("/tmp/goblin_fetch_abc123.txt"));
    ...
    fn test_skill_operation() {
        let fixture = ToolOperation::Skill {
            output: goblin_domain::Skill::new(
                "test-skill",
                "This is a test skill command with instructions",
                "A test skill for demonstration",
            )
            .path("/home/user/.goblin/skills/test-skill")
            .resources(vec![
                PathBuf::from("/home/user/.goblin/skills/test-skill/resource1.txt"),
                PathBuf::from("/home/user/.goblin/skills/test-skill/resource2.md"),
            ]),
        };
    ...
    fn test_fs_read_image_with_vision_model() {
        ...
        match &actual.values[0] {
            goblin_domain::ToolValue::Image(_) => (), // Expected
