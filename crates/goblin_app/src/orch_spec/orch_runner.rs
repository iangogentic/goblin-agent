...
impl Runner {
    fn new(setup: &TestContext) -> Self {
        ...
        goblin_embed::register_templates(&mut hb, &TEMPLATE_DIR);
...
impl AgentService for Runner {
    async fn chat_agent(
        &self,
        _id: &goblin_domain::ModelId,
        context: goblin_domain::Context,
        _provider_id: Option<ProviderId>,
    ) -> goblin_domain::ResultStream<ChatCompletionMessage, anyhow::Error> {
    ...
    async fn call(
        &self,
        _: &goblin_domain::Agent,
        _: &goblin_domain::ToolCallContext,
        test_call: goblin_domain::ToolCallFull,
    ) -> goblin_domain::ToolResult {
...
impl TemplateService for Runner {
    ...
    async fn render_template<V: serde::Serialize + Send + Sync>(
        &self,
        template: goblin_domain::Template<V>,
        object: &V,
...
impl AttachmentService for Runner {
    async fn attachments(&self, _url: &str) -> anyhow::Result<Vec<goblin_domain::Attachment>> {
...
impl SkillFetchService for Runner {
    async fn fetch_skill(&self, _skill_name: String) -> anyhow::Result<goblin_domain::Skill> {
