...
use goblin_domain::{ContextMessage, Image};
...
impl TryFrom<goblin_domain::Context> for Request {
    type Error = anyhow::Error;
    fn try_from(request: goblin_domain::Context) -> std::result::Result<Self, Self::Error> {
        let system_messages = request
            .messages
            .iter()
            .filter_map(|msg| match &**msg {
                ContextMessage::Text(msg) if msg.has_role(goblin_domain::Role::System) => {
                    Some(SystemMessage {
        ...
        Ok(Self {
            messages: request
                .messages
                .into_iter()
                .filter(|message| !message.has_role(goblin_domain::Role::System))
            ...
            output_format: request.response_format.and_then(|rf| match rf {
                goblin_domain::ResponseFormat::Text => {
        ...
        Ok(match value {
            ContextMessage::Text(chat_message) => {
                ...
                match chat_message.role {
                    goblin_domain::Role::User => Message { role: Role::User, content },
                    goblin_domain::Role::Assistant => Message { role: Role::Assistant, content },
                    goblin_domain::Role::System => {
                        ...
                        return Err(
                            goblin_domain::Error::UnsupportedRole("System".to_string()).into()
                        );
                    }
                }
            }
...
impl TryFrom<goblin_domain::ToolCallFull> for Content {
    type Error = anyhow::Error;
    fn try_from(value: goblin_domain::ToolCallFull) -> std::result::Result<Self, Self::Error> {
        let call_id = value
            .call_id
            .as_ref()
            .ok_or(goblin_domain::Error::ToolCallMissingId)?;
...
impl TryFrom<goblin_domain::ToolResult> for Content {
    type Error = anyhow::Error;
    fn try_from(value: goblin_domain::ToolResult) -> std::result::Result<Self, Self::Error> {
        let call_id = value
            .call_id
            .as_ref()
            .ok_or(goblin_domain::Error::ToolCallMissingId)?;
...
impl From<goblin_domain::ToolChoice> for ToolChoice {
    fn from(value: goblin_domain::ToolChoice) -> Self {
        match value {
            goblin_domain::ToolChoice::Auto => ToolChoice::Auto { disable_parallel_tool_use: None },
            goblin_domain::ToolChoice::Call(tool_name) => {
                ToolChoice::Tool { name: tool_name.to_string(), disable_parallel_tool_use: None }
            }
            goblin_domain::ToolChoice::Required => {
                ToolChoice::Any { disable_parallel_tool_use: None }
            }
            goblin_domain::ToolChoice::None => ToolChoice::Auto { disable_parallel_tool_use: None },
        }
    }
}
...
impl TryFrom<goblin_domain::ToolDefinition> for ToolDefinition {
    type Error = anyhow::Error;
    fn try_from(value: goblin_domain::ToolDefinition) -> std::result::Result<Self, Self::Error> {
        Ok(ToolDefinition {
            name: value.name.to_string(),
