...
use goblin_domain::{Context, ContextMessage};
use serde::Serialize;
...
impl From<Context> for Request {
    fn from(context: Context) -> Self {
        ...
        let system_parts: Vec<Part> = context
            .messages
            .iter()
            .filter(|msg| msg.has_role(goblin_domain::Role::System))
            .filter_map(|msg| {
        ...
        let generation_config = Some(GenerationConfig {
            ...
            response_mime_type: context.response_format.as_ref().and_then(|rf| match rf {
                goblin_domain::ResponseFormat::JsonSchema(_) => Some("application/json".to_string()),
                _ => None,
            }),
            response_schema: context.response_format.and_then(|rf| match rf {
                goblin_domain::ResponseFormat::JsonSchema(schema) => {
...
impl From<goblin_domain::ToolChoice> for ToolConfig {
    fn from(choice: goblin_domain::ToolChoice) -> Self {
        let (mode, allowed_function_names) = match choice {
            goblin_domain::ToolChoice::Auto => (FunctionCallingMode::Auto, None),
            goblin_domain::ToolChoice::None => (FunctionCallingMode::None, None),
            goblin_domain::ToolChoice::Required => (FunctionCallingMode::Any, None),
            goblin_domain::ToolChoice::Call(name) => {
...
impl From<goblin_domain::ToolDefinition> for FunctionDeclaration {
    fn from(tool: goblin_domain::ToolDefinition) -> Self {
...
impl From<goblin_domain::TextMessage> for Content {
    fn from(text_message: goblin_domain::TextMessage) -> Self {
        let role = match text_message.role {
            goblin_domain::Role::User => Some(Role::User),
            goblin_domain::Role::Assistant => Some(Role::Model),
            goblin_domain::Role::System => None, // System messages are handled separately
        };
...
impl From<goblin_domain::ToolResult> for Content {
    fn from(tool_result: goblin_domain::ToolResult) -> Self {
        Content {
...
impl From<goblin_domain::Image> for Content {
    fn from(image: goblin_domain::Image) -> Self {
        Content { role: Some(Role::User), parts: vec![Part::from(image)] }
    }
}

impl From<goblin_domain::ToolCallFull> for Part {
    fn from(tool_call: goblin_domain::ToolCallFull) -> Self {
        Part::FunctionCall {
            function_call: FunctionCallData {
...
impl From<goblin_domain::ToolResult> for Part {
    fn from(tool_result: goblin_domain::ToolResult) -> Self {
...
impl From<goblin_domain::Image> for Part {
    fn from(image: goblin_domain::Image) -> Self {
        Part::Image {
            inline_data: ImageSource {
...
#[cfg(test)]
mod tests {
    use goblin_domain::{ToolCallArguments, ToolCallFull, ToolCallId, ToolName, ToolResult};
    ...
    fn test_consecutive_tool_results_grouped() {
        use goblin_domain::{Context, ContextMessage, ModelId};
    ...
    fn test_text_message_conversion() {
        use goblin_domain::{Role, TextMessage};
    ...
    fn test_response_schema_strips_dollar_schema() {
        use goblin_domain::{Context, ResponseFormat};
        use schemars::schema_for;
    ...
    fn test_tool_result_part_conversion() {
        use goblin_domain::{ToolCallId, ToolName, ToolResult};
