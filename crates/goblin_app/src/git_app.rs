        ...
        + ProviderAuthService,
{
    ...
    ///
    /// Sets the user as the author (from git config) and GoblinCode as the
    /// committer. This properly attributes the commit to both the user and
    /// GoblinCode using Git's author/committer distinction.
    ///
    /// # Arguments
    ///
    ...
    pub async fn commit(&self, message: String, has_staged_files: bool) -> Result<CommitResult> {
        ...
        // Set GoblinCode as the committer while keeping the user as the author
        ...
        let commit_command = format!(
            "GIT_COMMITTER_NAME='GoblinCode' GIT_COMMITTER_EMAIL='noreply@goblincode.dev' git commit {flags} -m '{escaped_message}'"
        );
    ...
    async fn generate_message_from_diff(&self, ctx: DiffContext) -> Result<CommitMessageDetails> {
        ...
        let rendered_prompt = self
            .services
            .render_template(Template::new("{{> goblin-commit-message-prompt.md }}"), &())
            .await?;
        ...
        let context = goblin_domain::Context::default()
