...
/// Builds a [`TemplateConfig`] from a [`GoblinConfig`].
///
...
pub(crate) fn build_template_config(config: &GoblinConfig) -> goblin_domain::TemplateConfig {
    goblin_domain::TemplateConfig {
...
/// GoblinApp handles the core chat functionality by orchestrating various
/// services. It encapsulates the complex logic previously contained in the
/// GoblinAPI chat method.
pub struct GoblinApp<S> {
    services: Arc<S>,
    tool_registry: ToolRegistry<S>,
}

impl<S: Services> GoblinApp<S> {
    /// Creates a new GoblinApp instance with the provided services.
    pub fn new(services: Arc<S>) -> Self {
        Self { tool_registry: ToolRegistry::new(services.clone()), services }
    }

    /// Executes a chat request and returns a stream of responses.
    /// This method contains the core chat logic extracted from GoblinAPI.
    pub async fn chat(
        &self,
        agent_id: AgentId,
        chat: ChatRequest,
        ...
        let goblin_config = services.get_config();
        ...
        let conversation =
            SystemPrompt::new(self.services.clone(), environment.clone(), agent.clone())
                ...
                .models(models.clone())
                .files(files.clone())
                .max_extensions(goblin_config.max_extensions)
                .template_config(build_template_config(&goblin_config))
        ...
        let retry_config = goblin_config.retry.clone().unwrap_or_default();
    ...
    pub async fn compact_conversation(
        ...
        let mut conversation = self
            ...
            .await?
            .ok_or_else(|| goblin_domain::Error::ConversationNotFound(*conversation_id))?;
        ...
        let goblin_config = self.services.get_config();
