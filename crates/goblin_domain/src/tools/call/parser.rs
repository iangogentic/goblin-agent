...
fn parse_tool_call(input: &str) -> IResult<&str, ToolCallParsed> {
    ...
    let (input, _) = tag("<goblin_tool_call>").parse(input)?;
    let (input, _) = multispace0(input)?; // Handle whitespace after <goblin_tool_call>

    // Match the tool name tags: <goblin_tool_name>
    let (input, _) = tag("<").parse(input)?;
...
fn find_next_tool_call(input: &str) -> IResult<&str, &str> {
    ...
    let (remaining, _) = take_until("<goblin_tool_call>").parse(input)?;
    Ok((remaining, ""))
}
...
mod tests {
    ...
    impl ToolCallBuilder {
        ...
        fn build_xml(&self) -> String {
            let mut xml = String::from("<goblin_tool_call>");
            ...
            xml.push_str(&format!("</{}></goblin_tool_call>", self.name));
            xml
        }
    ...
    fn test_parse_new_tool_call_format() {
        let input = r#"<goblin_tool_call><fs_search><path>/test/path</path><regex>test</regex></fs_search></goblin_tool_call>"#;
    ...
    fn test_parse_with_newlines() {
        let input = [
            "<goblin_tool_call><foo><p1>",
            "abc",
            "</p1></foo></goblin_tool_call>",
        ]
        .join("\n");
