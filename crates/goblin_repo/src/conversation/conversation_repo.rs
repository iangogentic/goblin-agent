...
use diesel::prelude::*;
use goblin_domain::{Conversation, ConversationId, ConversationRepository, WorkspaceHash};
...
mod tests {
    ...
    fn test_context_record_conversion_preserves_all_fields() {
        ...
        let reasoning = goblin_domain::ReasoningConfig {
        ...
        let messages = vec![
            ...
            .into(),
            goblin_domain::MessageEntry {
                message: ContextMessage::Text(goblin_domain::TextMessage {
                    role: Role::Assistant,
                    ...
                    tool_calls: Some(vec![ToolCallFull {
                        ...
                        arguments: goblin_domain::ToolCallArguments::from(
                            serde_json::json!({"param": "value"}),
                        ),
                        thought_signature: None,
                    }]),
                    model: Some(goblin_domain::ModelId::from("gpt-4")),
                    thought_signature: None,
                ...
                usage: Some(Usage {
                    prompt_tokens: goblin_domain::TokenCount::Actual(100),
                    completion_tokens: goblin_domain::TokenCount::Actual(50),
                    total_tokens: goblin_domain::TokenCount::Actual(150),
                    cached_tokens: goblin_domain::TokenCount::Actual(0),
                    cost: Some(0.001),
                }),
            },
        ];
        ...
        let fixture = Context::default()
            ...
            .temperature(goblin_domain::Temperature::new(0.7).unwrap())
            .top_p(goblin_domain::TopP::new(0.9).unwrap())
            .top_k(goblin_domain::TopK::new(50).unwrap())
    ...
    fn test_legacy_tool_value_pair_deserialization() {
        ...
        let actual: goblin_domain::ToolOutput = record.try_into().unwrap();
        ...
        assert_eq!(
            actual.values[0],
            goblin_domain::ToolValue::Text("XML content for LLM".to_string())
        );
    }
    ...
    fn test_legacy_tool_value_markdown_deserialization() {
        ...
        assert_eq!(
            actual.values[0],
            goblin_domain::ToolValue::Text("# Heading - Some bold text".to_string())
        );
    }
    ...
    fn test_legacy_tool_value_file_diff_deserialization() {
        ...
        assert_eq!(
            actual.values[0],
            goblin_domain::ToolValue::Text("[File diff: /src/main.rs]".to_string())
        );
    }
}
