...
//! `goblin_domain` counterparts for compile-time safety while keeping the
//! storage layer independent from domain model changes.

use anyhow::Context as _;
use goblin_domain::{Context, ConversationId};
...
impl From<&goblin_domain::ModelId> for ModelIdRecord {
    fn from(id: &goblin_domain::ModelId) -> Self {
        Self(id.to_string())
    }
}

impl From<ModelIdRecord> for goblin_domain::ModelId {
    fn from(record: ModelIdRecord) -> Self {
        goblin_domain::ModelId::from(record.0)
    }
}
...
impl From<&goblin_domain::Image> for ImageRecord {
    fn from(image: &goblin_domain::Image) -> Self {
        Self {
...
impl From<ImageRecord> for goblin_domain::Image {
    fn from(record: ImageRecord) -> Self {
        goblin_domain::Image::new_base64(
            record
                .url
...
impl From<&goblin_domain::ToolCallId> for ToolCallIdRecord {
    fn from(id: &goblin_domain::ToolCallId) -> Self {
        Self(id.as_str().to_string())
    }
}

impl From<ToolCallIdRecord> for goblin_domain::ToolCallId {
    fn from(record: ToolCallIdRecord) -> Self {
        goblin_domain::ToolCallId::new(record.0)
    }
}
...
impl From<&goblin_domain::ToolCallArguments> for ToolCallArgumentsRecord {
    fn from(args: &goblin_domain::ToolCallArguments) -> Self {
        // Serialize to JSON to capture both Parsed and Unparsed variants
        Self(serde_json::to_value(args).unwrap_or_default())
    }
}

impl From<ToolCallArgumentsRecord> for goblin_domain::ToolCallArguments {
...
impl From<&goblin_domain::ToolName> for ToolNameRecord {
    fn from(name: &goblin_domain::ToolName) -> Self {
        Self(name.to_string())
    }
}

impl From<ToolNameRecord> for goblin_domain::ToolName {
    fn from(record: ToolNameRecord) -> Self {
        goblin_domain::ToolName::new(record.0)
    }
}
...
impl From<&goblin_domain::ToolCallFull> for ToolCallFullRecord {
    fn from(call: &goblin_domain::ToolCallFull) -> Self {
        Self {
...
impl From<ToolCallFullRecord> for goblin_domain::ToolCallFull {
    fn from(record: ToolCallFullRecord) -> Self {
        goblin_domain::ToolCallFull {
...
impl From<&goblin_domain::ReasoningFull> for ReasoningFullRecord {
    fn from(reasoning: &goblin_domain::ReasoningFull) -> Self {
        Self {
...
impl From<ReasoningFullRecord> for goblin_domain::ReasoningFull {
    fn from(record: ReasoningFullRecord) -> Self {
        goblin_domain::ReasoningFull {
            text: record.text,
...
impl From<&goblin_domain::TokenCount> for TokenCountRecord {
    fn from(count: &goblin_domain::TokenCount) -> Self {
        match count {
            goblin_domain::TokenCount::Actual(n) => Self::Actual(*n),
            goblin_domain::TokenCount::Approx(n) => Self::Approx(*n),
        }
    }
}

impl From<TokenCountRecord> for goblin_domain::TokenCount {
...
impl From<&goblin_domain::Usage> for UsageRecord {
    fn from(usage: &goblin_domain::Usage) -> Self {
        Self {
...
impl From<UsageRecord> for goblin_domain::Usage {
...
impl From<&goblin_domain::EventValue> for EventValueRecord {
    fn from(event: &goblin_domain::EventValue) -> Self {
        Self(serde_json::to_value(event).unwrap_or_default())
    }
}

impl TryFrom<EventValueRecord> for goblin_domain::EventValue {
...
impl From<&goblin_domain::Role> for RoleRecord {
    fn from(role: &goblin_domain::Role) -> Self {
        match role {
            goblin_domain::Role::System => Self::System,
            goblin_domain::Role::User => Self::User,
            goblin_domain::Role::Assistant => Self::Assistant,
        }
    }
}

impl From<RoleRecord> for goblin_domain::Role {
...
impl From<&goblin_domain::TextMessage> for TextMessageRecord {
    fn from(msg: &goblin_domain::TextMessage) -> Self {
        Self {
...
impl TryFrom<TextMessageRecord> for goblin_domain::TextMessage {
    type Error = anyhow::Error;

    fn try_from(record: TextMessageRecord) -> anyhow::Result<Self> {
        Ok(goblin_domain::TextMessage {
...
impl From<&goblin_domain::ToolValue> for ToolValueRecord {
    fn from(value: &goblin_domain::ToolValue) -> Self {
        match value {
            goblin_domain::ToolValue::Text(text) => Self::Text(text.clone()),
            goblin_domain::ToolValue::AI { value, conversation_id } => Self::AI {
                value: value.clone(),
                conversation_id: conversation_id.into_string(),
            },
            goblin_domain::ToolValue::Image(img) => Self::Image(ImageRecord::from(img)),
            goblin_domain::ToolValue::Empty => Self::Empty,
        }
    }
}

impl TryFrom<ToolValueRecord> for goblin_domain::ToolValue {
    type Error = anyhow::Error;
...
impl From<&goblin_domain::ToolOutput> for ToolOutputRecord {
    fn from(output: &goblin_domain::ToolOutput) -> Self {
        Self {
...
impl TryFrom<ToolOutputRecord> for goblin_domain::ToolOutput {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ToolOutputRecord) -> anyhow::Result<Self> {
        ...
        Ok(goblin_domain::ToolOutput { is_error: record.is_error, values: values? })
    }
}
...
impl From<&goblin_domain::ToolResult> for ToolResultRecord {
    fn from(result: &goblin_domain::ToolResult) -> Self {
        Self {
...
impl TryFrom<ToolResultRecord> for goblin_domain::ToolResult {
    type Error = anyhow::Error;

    fn try_from(record: ToolResultRecord) -> anyhow::Result<Self> {
        Ok(goblin_domain::ToolResult {
            name: record.name.into(),
            call_id: record.call_id.map(Into::into),
...
impl From<&goblin_domain::ContextMessage> for ContextMessageValueRecord {
    fn from(value: &goblin_domain::ContextMessage) -> Self {
        match value {
            goblin_domain::ContextMessage::Text(msg) => Self::Text(TextMessageRecord::from(msg)),
            goblin_domain::ContextMessage::Tool(result) => {
                Self::Tool(ToolResultRecord::from(result))
            }
            goblin_domain::ContextMessage::Image(img) => Self::Image(ImageRecord::from(img)),
        }
    }
}

impl TryFrom<ContextMessageValueRecord> for goblin_domain::ContextMessage {
    type Error = anyhow::Error;
...
impl From<&goblin_domain::MessageEntry> for ContextMessageRecord {
    fn from(msg: &goblin_domain::MessageEntry) -> Self {
        Self {
...
impl TryFrom<ContextMessageRecord> for goblin_domain::MessageEntry {
    type Error = anyhow::Error;

    fn try_from(record: ContextMessageRecord) -> anyhow::Result<Self> {
        Ok(goblin_domain::MessageEntry {
...
impl From<&goblin_domain::ToolDefinition> for ToolDefinitionRecord {
    fn from(def: &goblin_domain::ToolDefinition) -> Self {
        Self {
...
impl TryFrom<ToolDefinitionRecord> for goblin_domain::ToolDefinition {
    type Error = anyhow::Error;

    fn try_from(record: ToolDefinitionRecord) -> anyhow::Result<Self> {
        Ok(goblin_domain::ToolDefinition {
            name: record.name.into(),
...
impl From<&goblin_domain::ToolChoice> for ToolChoiceRecord {
    fn from(choice: &goblin_domain::ToolChoice) -> Self {
        match choice {
            goblin_domain::ToolChoice::None => Self::None,
            goblin_domain::ToolChoice::Auto => Self::Auto,
            goblin_domain::ToolChoice::Required => Self::Required,
            goblin_domain::ToolChoice::Call(name) => Self::Call(ToolNameRecord::from(name)),
        }
    }
}

impl From<ToolChoiceRecord> for goblin_domain::ToolChoice {
...
impl From<&goblin_domain::Effort> for EffortRecord {
    fn from(effort: &goblin_domain::Effort) -> Self {
        match effort {
            goblin_domain::Effort::High => Self::High,
            goblin_domain::Effort::Medium => Self::Medium,
            goblin_domain::Effort::Low => Self::Low,
        }
    }
}

impl From<EffortRecord> for goblin_domain::Effort {
...
impl From<&goblin_domain::ReasoningConfig> for ReasoningConfigRecord {
    fn from(config: &goblin_domain::ReasoningConfig) -> Self {
        Self {
...
impl From<ReasoningConfigRecord> for goblin_domain::ReasoningConfig {
    fn from(record: ReasoningConfigRecord) -> Self {
        goblin_domain::ReasoningConfig {
...
impl TryFrom<ContextRecord> for Context {
    ...
    fn try_from(record: ContextRecord) -> anyhow::Result<Self> {
        ...
        Ok(Context {
            ...
            temperature: record
                .temperature
                .map(goblin_domain::Temperature::new_unchecked),
            top_p: record.top_p.map(goblin_domain::TopP::new_unchecked),
            top_k: record.top_k.map(goblin_domain::TopK::new_unchecked),
...
pub(super) struct FileChangeMetricsRecord {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    tool: Option<goblin_domain::ToolKind>,
}

impl From<&goblin_domain::FileOperation> for FileChangeMetricsRecord {
    fn from(metrics: &goblin_domain::FileOperation) -> Self {
        Self {
...
impl From<FileChangeMetricsRecord> for goblin_domain::FileOperation {
    fn from(record: FileChangeMetricsRecord) -> Self {
        // Use Write as default tool for old records without tool field
        let tool = record.tool.unwrap_or(goblin_domain::ToolKind::Write);
        Self::new(tool)
...
impl From<&goblin_domain::Metrics> for MetricsRecord {
    fn from(metrics: &goblin_domain::Metrics) -> Self {
        Self {
...
impl From<MetricsRecord> for goblin_domain::Metrics {
    fn from(record: MetricsRecord) -> Self {
        let file_operations: std::collections::HashMap<String, goblin_domain::FileOperation> =
            record
                .files_changed
                .into_iter()
        ...
        let files_accessed = if record.files_accessed.is_empty() {
            ...
            file_operations
                .iter()
                .filter(|(_, op)| op.tool == goblin_domain::ToolKind::Read)
...
impl ConversationRecord {
    ...
    pub fn new(
        conversation: goblin_domain::Conversation,
        workspace_id: goblin_domain::WorkspaceHash,
    ) -> Self {
...
impl TryFrom<ConversationRecord> for goblin_domain::Conversation {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ConversationRecord) -> anyhow::Result<Self> {
        ...
        let metrics = record
            ...
            .map(goblin_domain::Metrics::from)
            .unwrap_or_else(|| {
                goblin_domain::Metrics::default().started_at(record.created_at.and_utc())
            });

        Ok(goblin_domain::Conversation::new(id)
            .context(context)
            .title(record.title)
            .metrics(metrics)
            .metadata(
                goblin_domain::MetaData::new(record.created_at.and_utc())
