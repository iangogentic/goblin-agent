...
use goblin_app::{DirectoryReaderInfra, EnvironmentInfra, FileInfoInfra};
use goblin_domain::Template;
...
/// - **Global agents**: `{HOME}/.goblin/agents/*.md`
/// - **CWD agents**: `./.goblin/agents/*.md` (relative to current working
///   directory)
///
/// Missing directories are handled gracefully and don't prevent loading from
/// other sources.
pub struct GoblinAgentRepository<I> {
    infra: Arc<I>,
}

impl<I> GoblinAgentRepository<I> {
    pub fn new(infra: Arc<I>) -> Self {
        Self { infra }
    }
}

impl<I: FileInfoInfra + EnvironmentInfra + DirectoryReaderInfra> GoblinAgentRepository<I> {
...
mod tests {
    ...
    async fn test_parse_basic_agent() {
        let content = goblin_test_kit::fixture!("/src/fixtures/agents/basic.md").await;
    ...
    async fn test_parse_advanced_agent() {
        let content = goblin_test_kit::fixture!("/src/fixtures/agents/advanced.md").await;
