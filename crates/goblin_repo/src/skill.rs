...
use anyhow::Context;
use goblin_app::domain::Skill;
use goblin_app::{EnvironmentInfra, FileInfoInfra, FileReaderInfra, Walker, WalkerInfra};
use goblin_domain::SkillRepository;
...
/// 2. Global custom skills (from ~/goblin/skills/ directory)
/// 3. Project-local skills (from .goblin/skills/ directory in current working
///    directory)
///
/// ## Skill Precedence
...
/// - **Global skills**: `~/goblin/skills/<skill-name>/SKILL.md`
/// - **CWD skills**: `./.goblin/skills/<skill-name>/SKILL.md` (relative to
///   current working directory)
///
/// Missing directories are handled gracefully and don't prevent loading from
/// other sources.
pub struct GoblinSkillRepository<I> {
    infra: Arc<I>,
}

impl<I> GoblinSkillRepository<I> {
    ...
    fn load_builtin_skills(&self) -> Vec<Skill> {
        let builtin_skills = vec![
            (
                "goblin://skills/create-skill/SKILL.md",
                include_str!("skills/create-skill/SKILL.md"),
            ),
            (
                "goblin://skills/execute-plan/SKILL.md",
                include_str!("skills/execute-plan/SKILL.md"),
            ),
            (
                "goblin://skills/github-pr-description/SKILL.md",
...
impl<I: FileInfoInfra + EnvironmentInfra + FileReaderInfra + WalkerInfra> GoblinSkillRepository<I> {
    ...
    fn render_skill(&self, skill: Skill, env: &goblin_domain::Environment) -> Skill {
...
#[cfg(test)]
mod tests {
    use goblin_infra::GoblinInfra;
    use pretty_assertions::assert_eq;

    use super::*;

    fn fixture_skill_repo() -> (GoblinSkillRepository<GoblinInfra>, std::path::PathBuf) {
        let skill_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/fixtures/skills_with_resources");
        let infra = Arc::new(GoblinInfra::new(std::env::current_dir().unwrap()));
        let repo = GoblinSkillRepository::new(infra);
        (repo, skill_dir)
    }
    ...
    fn test_load_builtin_skills() {
        // Fixture
        let repo = GoblinSkillRepository { infra: Arc::new(()) };
        ...
        assert_eq!(
            create_skill.path,
            Some(std::path::Path::new("goblin://skills/create-skill/SKILL.md").to_path_buf())
        );
        assert_eq!(
        ...
        assert_eq!(
            execute_plan.path,
            Some(std::path::Path::new("goblin://skills/execute-plan/SKILL.md").to_path_buf())
        );
        assert!(
            execute_plan
                .description
        ...
        assert_eq!(
            pr_description.path,
            Some(
                std::path::Path::new("goblin://skills/github-pr-description/SKILL.md").to_path_buf()
            )
        );
    ...
    async fn test_extract_skill_with_valid_metadata() {
        ...
        let content =
            goblin_test_kit::fixture!("/src/fixtures/skills/with_name_and_description.md").await;
    ...
    async fn test_extract_skill_with_incomplete_metadata() {
        // Fixture
        let content = goblin_test_kit::fixture!("/src/fixtures/skills/with_name_only.md").await;
