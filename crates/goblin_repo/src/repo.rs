...
// Re-export CacacheStorage from goblin_infra
pub use goblin_infra::CacacheStorage;
use reqwest::Response;
use reqwest::header::HeaderMap;
use reqwest_eventsource::EventSource;
use url::Url;

use crate::agent::GoblinAgentRepository;
use crate::context_engine::GoblinContextEngineRepository;
use crate::conversation::ConversationRepositoryImpl;
use crate::database::{DatabasePool, PoolConfig};
use crate::fs_snap::GoblinFileSnapshotService;
use crate::fuzzy_search::GoblinFuzzySearchRepository;
use crate::provider::{GoblinChatRepository, GoblinProviderRepository};
use crate::skill::GoblinSkillRepository;
use crate::validation::GoblinValidationRepository;
...
#[derive(Clone)]
pub struct GoblinRepo<F> {
    infra: Arc<F>,
    file_snapshot_service: Arc<GoblinFileSnapshotService>,
    conversation_repository: Arc<ConversationRepositoryImpl>,
    mcp_cache_repository: Arc<CacacheStorage>,
    provider_repository: Arc<GoblinProviderRepository<F>>,
    chat_repository: Arc<GoblinChatRepository<F>>,
    codebase_repo: Arc<GoblinContextEngineRepository<F>>,
    agent_repository: Arc<GoblinAgentRepository<F>>,
    skill_repository: Arc<GoblinSkillRepository<F>>,
    validation_repository: Arc<GoblinValidationRepository<F>>,
    fuzzy_search_repository: Arc<GoblinFuzzySearchRepository<F>>,
}

impl<F: EnvironmentInfra + FileReaderInfra + FileWriterInfra + GrpcInfra + HttpInfra> GoblinRepo<F> {
    pub fn new(infra: Arc<F>) -> Self {
        let env = infra.get_environment();
        let file_snapshot_service = Arc::new(GoblinFileSnapshotService::new(env.clone()));
        let db_pool =
        ...
        let provider_repository = Arc::new(GoblinProviderRepository::new(infra.clone()));
        let chat_repository = Arc::new(GoblinChatRepository::new(infra.clone()));

        let codebase_repo = Arc::new(GoblinContextEngineRepository::new(infra.clone()));
        let agent_repository = Arc::new(GoblinAgentRepository::new(infra.clone()));
        let skill_repository = Arc::new(GoblinSkillRepository::new(infra.clone()));
        let validation_repository = Arc::new(GoblinValidationRepository::new(infra.clone()));
        let fuzzy_search_repository = Arc::new(GoblinFuzzySearchRepository::new(infra.clone()));
        Self {
            infra,
...
impl<F: Send + Sync> SnapshotRepository for GoblinRepo<F> {
...
#[async_trait::async_trait]
impl<F: Send + Sync> ConversationRepository for GoblinRepo<F> {
...
impl<F: EnvironmentInfra + FileReaderInfra + FileWriterInfra + HttpInfra + Send + Sync>
    ChatRepository for GoblinRepo<F>
{
    async fn chat(
        &self,
        model_id: &ModelId,
...
#[async_trait::async_trait]
impl<F: EnvironmentInfra + FileReaderInfra + FileWriterInfra + HttpInfra + Send + Sync>
    ProviderRepository for GoblinRepo<F>
{
    async fn get_all_providers(&self) -> anyhow::Result<Vec<AnyProvider>> {
        self.provider_repository.get_all_providers().await
    }

    async fn get_provider(&self, id: ProviderId) -> anyhow::Result<goblin_domain::ProviderTemplate> {
...
#[async_trait::async_trait]
impl<F: EnvironmentInfra + Send + Sync> EnvironmentInfra for GoblinRepo<F> {
...
#[async_trait::async_trait]
impl<F: Send + Sync> KVStore for GoblinRepo<F> {
...
#[async_trait::async_trait]
impl<F: HttpInfra> HttpInfra for GoblinRepo<F> {
...
#[async_trait::async_trait]
impl<F> FileReaderInfra for GoblinRepo<F>
where
...
#[async_trait::async_trait]
impl<F> WalkerInfra for GoblinRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileWriterInfra for GoblinRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileInfoInfra for GoblinRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileDirectoryInfra for GoblinRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileRemoverInfra for GoblinRepo<F>
where
...
#[async_trait::async_trait]
impl<F> DirectoryReaderInfra for GoblinRepo<F>
where
...
#[async_trait::async_trait]
impl<F> UserInfra for GoblinRepo<F>
where
...
#[async_trait::async_trait]
impl<F> McpServerInfra for GoblinRepo<F>
where
...
#[async_trait::async_trait]
impl<F> CommandInfra for GoblinRepo<F>
where
    ...
    for GoblinRepo<F>
{
    async fn get_agents(
        &self,
        provider_id: goblin_domain::ProviderId,
        model_id: goblin_domain::ModelId,
    ) -> anyhow::Result<Vec<goblin_domain::Agent>> {
...
impl<F: FileInfoInfra + EnvironmentInfra + FileReaderInfra + WalkerInfra + Send + Sync>
    SkillRepository for GoblinRepo<F>
{
...
impl<F: StrategyFactory> StrategyFactory for GoblinRepo<F> {
    type Strategy = F::Strategy;

    fn create_auth_strategy(
        &self,
        provider_id: ProviderId,
        auth_method: goblin_domain::AuthMethod,
        required_params: Vec<goblin_domain::URLParamSpec>,
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> goblin_domain::WorkspaceIndexRepository for GoblinRepo<F> {
    async fn authenticate(&self) -> anyhow::Result<goblin_domain::WorkspaceAuth> {
    ...
    ) -> anyhow::Result<goblin_domain::WorkspaceId> {
        self.codebase_repo
    ...
    async fn upload_files(
        &self,
        upload: &goblin_domain::FileUpload,
        auth_token: &goblin_domain::ApiKey,
    ) -> anyhow::Result<goblin_domain::FileUploadInfo> {
        self.codebase_repo.upload_files(upload, auth_token).await
    }

    async fn search(
        &self,
        query: &goblin_domain::CodeSearchQuery<'_>,
        auth_token: &goblin_domain::ApiKey,
    ) -> anyhow::Result<Vec<goblin_domain::Node>> {
    ...
    async fn list_workspaces(
        &self,
        auth_token: &goblin_domain::ApiKey,
    ) -> anyhow::Result<Vec<goblin_domain::WorkspaceInfo>> {
        self.codebase_repo.list_workspaces(auth_token).await
    }

    async fn get_workspace(
        &self,
        workspace_id: &goblin_domain::WorkspaceId,
        auth_token: &goblin_domain::ApiKey,
    ) -> anyhow::Result<Option<goblin_domain::WorkspaceInfo>> {
        self.codebase_repo
    ...
    async fn list_workspace_files(
        &self,
        workspace: &goblin_domain::WorkspaceFiles,
        auth_token: &goblin_domain::ApiKey,
    ) -> anyhow::Result<Vec<goblin_domain::FileHash>> {
        self.codebase_repo
    ...
    async fn delete_files(
        &self,
        deletion: &goblin_domain::FileDeletion,
        auth_token: &goblin_domain::ApiKey,
    ) -> anyhow::Result<()> {
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> goblin_domain::ValidationRepository for GoblinRepo<F> {
    async fn validate_file(
        &self,
        path: impl AsRef<std::path::Path> + Send,
        content: &str,
    ) -> anyhow::Result<Vec<goblin_domain::SyntaxError>> {
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> FuzzySearchRepository for GoblinRepo<F> {
...
impl<F: GrpcInfra> GrpcInfra for GoblinRepo<F> {
...
impl<F: goblin_domain::ConsoleWriter> goblin_domain::ConsoleWriter for GoblinRepo<F> {
