...
pub struct GoblinContextEngineRepository<I> {
    infra: Arc<I>,
}

impl<I> GoblinContextEngineRepository<I> {
...
#[async_trait]
impl<I: GrpcInfra> WorkspaceIndexRepository for GoblinContextEngineRepository<I> {
    ...
    async fn search(
        &self,
        search_query: &goblin_domain::CodeSearchQuery<'_>,
        auth_token: &goblin_domain::ApiKey,
    ) -> Result<Vec<Node>> {
            ...
            .filter_map(|query_item| {
                ...
                let code_node = match node_data.kind? {
                    node_data::Kind::FileChunk(chunk) => {
                        goblin_domain::NodeData::FileChunk(goblin_domain::FileChunk {
                    ...
                    node_data::Kind::File(file) => {
                        goblin_domain::NodeData::File(goblin_domain::FileNode {
                            file_path: file.path,
                    ...
                    node_data::Kind::FileRef(file_ref) => {
                        goblin_domain::NodeData::FileRef(goblin_domain::FileRef {
                            file_path: file_ref.path,
                            file_hash: file_ref.file_hash,
                        })
                    }
                    node_data::Kind::Note(note) => {
                        goblin_domain::NodeData::Note(goblin_domain::Note { content: note.content })
                    }
                    node_data::Kind::Task(task) => {
                        goblin_domain::NodeData::Task(goblin_domain::Task { task: task.task })
                    }
                };
    ...
    async fn list_workspace_files(
        ...
        auth_token: &goblin_domain::ApiKey,
    ) -> Result<Vec<goblin_domain::FileHash>> {
