...
use goblin_app::{EnvironmentInfra, HttpInfra};
use goblin_domain::{ChatRepository, Provider, ProviderId};
use goblin_infra::CacacheStorage;
...
pub struct GoblinChatRepository<F> {
    router: Arc<ProviderRouter<F>>,
    model_cache: Arc<CacacheStorage>,
    bg_refresh: BgRefresh,
}

impl<F: EnvironmentInfra + HttpInfra> GoblinChatRepository<F> {
    /// Creates a new GoblinChatRepository with the given infrastructure.
    ///
    /// # Arguments
    ///
...
impl<F: EnvironmentInfra + HttpInfra + Sync> ChatRepository for GoblinChatRepository<F> {
    async fn chat(
        &self,
        model_id: &ModelId,
        context: Context,
