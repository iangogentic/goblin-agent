...
use goblin_domain::{BoxStream, ResultStream};
            ...
            match item {
                ...
                oai::OutputItem::Reasoning(reasoning) => {
                    ...
                    if let Some(encrypted_content) = &reasoning.encrypted_content {
                        message =
                            message.add_reasoning_detail(goblin_domain::Reasoning::Full(vec![
...
fn retain_encrypted_reasoning_details(
    details: Option<Vec<goblin_domain::Reasoning>>,
) -> Option<Vec<goblin_domain::Reasoning>> {
    let details = details?;
    let encrypted: Vec<goblin_domain::Reasoning> = details
        .into_iter()
        .filter(|r| {
...
mod tests {
    ...
    use goblin_app::domain::{Content, FinishReason, Reasoning, ReasoningFull, TokenCount, Usage};
    use goblin_domain::{ChatCompletionMessage as Message, ToolCallId, ToolName};
    ...
    fn test_response_into_domain_preserves_commentary_phase() {
        ...
        assert_eq!(
            actual.phase,
            Some(goblin_app::domain::MessagePhase::Commentary)
        );
    ...
    fn test_response_into_domain_preserves_final_answer_phase() {
        ...
        assert_eq!(
            actual.phase,
            Some(goblin_app::domain::MessagePhase::FinalAnswer)
        );
    ...
    async fn test_stream_with_reasoning_text_delta() -> anyhow::Result<()> {
        ...
        assert_eq!(
            actual.reasoning_details,
            Some(vec![Reasoning::Part(vec![goblin_domain::ReasoningPart {
    ...
    async fn test_spark_style_stream_function_call_no_deltas() -> anyhow::Result<()> {
        ...
        let done = oai::ResponseFunctionCallArgumentsDoneEvent {
            ...
            name: Some("shell".to_string()),
            arguments: r#"{"command":"date \"+%Y-%m-%d\"","cwd":"/Users/amit/code-goblin","description":"Get current date","env":[],"keep_ansi":false}"#.to_string(),
        };
