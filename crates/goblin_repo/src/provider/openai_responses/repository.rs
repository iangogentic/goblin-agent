...
use goblin_domain::{BoxStream, ChatRepository, Provider};
    ...
    fn get_headers_for_conversation(&self, conversation_id: Option<&str>) -> Vec<(String, String)> {
        ...
        if let Some(api_key) = self
            ...
            .map(|c| match &c.auth_details {
                goblin_domain::AuthDetails::ApiKey(key) => key.as_str(),
                goblin_domain::AuthDetails::OAuthWithApiKey { api_key, .. } => api_key.as_str(),
                goblin_domain::AuthDetails::OAuth { tokens, .. } => tokens.access_token.as_str(),
                goblin_domain::AuthDetails::GoogleAdc(token) => token.as_str(),
            })
        {
        ...
        self.provider
            ...
            .for_each(|method| match method {
                goblin_domain::AuthMethod::ApiKey => {}
                goblin_domain::AuthMethod::OAuthDevice(oauth_config) => {
                ...
                goblin_domain::AuthMethod::OAuthCode(oauth_config) => {
                    if let Some(custom_headers) = &oauth_config.custom_headers {
                        custom_headers.iter().for_each(|(k, v)| {
                            headers.push((k.clone(), v.clone()));
                        });
                ...
                goblin_domain::AuthMethod::CodexDevice(oauth_config) => {
                    if let Some(custom_headers) = &oauth_config.custom_headers {
                        custom_headers.iter().for_each(|(k, v)| {
                            headers.push((k.clone(), v.clone()));
                        });
                ...
                goblin_domain::AuthMethod::GoogleAdc => {}
            });
        ...
        if self.provider.id == goblin_domain::ProviderId::CODEX {
            ...
            if let Some(account_id) = self.provider.credential.as_ref().and_then(|c| {
                let key: goblin_domain::URLParam = "chatgpt_account_id".to_string().into();
                    ...
                    Ok(Event::Message(msg)) => {
                        ...
                        match result {
                            ...
                            Ok(super::response::ResponsesStreamEvent::Ping { cost }) => {
                                let usage =
                                    goblin_domain::Usage { cost: Some(cost), ..Default::default() };
                                Some(Ok(super::response::StreamItem::Message(Box::new(
                                    ChatCompletionMessage::assistant(goblin_domain::Content::part(
                                        "",
                                    ))
                                    .usage(usage),
                                ))))
    ...
    E: std::fmt::Debug + std::fmt::Display + Send + Sync + 'static,
{
    ...
    if is_retryable {
        goblin_domain::Error::Retryable(error).into()
    } else {
        error
    }
}
...
impl<F: HttpInfra + 'static> ChatRepository for OpenAIResponsesResponseRepository<F> {
    ...
    async fn models(&self, provider: Provider<Url>) -> anyhow::Result<Vec<Model>> {
        match provider.models().cloned() {
            Some(goblin_domain::ModelSource::Hardcoded(models)) => Ok(models),
            Some(goblin_domain::ModelSource::Url(url)) => {
                ...
                let data: goblin_app::dto::openai::ListModelResponse =
...
mod tests {
    ...
    fn is_retryable(error: &anyhow::Error) -> bool {
        error
            .downcast_ref::<goblin_domain::Error>()
            .is_some_and(|error| matches!(error, goblin_domain::Error::Retryable(_)))
    }

    fn make_credential(provider_id: ProviderId, key: &str) -> Option<goblin_domain::AuthCredential> {
        Some(goblin_domain::AuthCredential {
            id: provider_id,
            auth_details: goblin_domain::AuthDetails::ApiKey(goblin_domain::ApiKey::from(
                key.to_string(),
            )),
    ...
    fn openai_responses(key: &str, url: &str) -> Provider<Url> {
        Provider {
            ...
            provider_type: goblin_domain::ProviderType::Llm,
            response: Some(ProviderResponse::OpenAI),
            url: Url::parse(url).unwrap(),
            credential: make_credential(ProviderId::OPENAI, key),
            custom_headers: None,
            auth_methods: vec![goblin_domain::AuthMethod::ApiKey],
            url_params: vec![],
            models: None,
        }
    }
    ...
    fn test_openai_responses_provider_new_with_oauth_with_api_key() {
        let provider = Provider {
            ...
            credential: Some(goblin_domain::AuthCredential {
                id: ProviderId::OPENAI,
                auth_details: goblin_domain::AuthDetails::OAuthWithApiKey {
                    tokens: goblin_domain::OAuthTokens::new(
                        "access-token",
                        None::<String>,
                        chrono::Utc::now() + chrono::Duration::hours(1),
                    ),
                    api_key: goblin_domain::ApiKey::from("oauth-key".to_string()),
                    config: goblin_domain::OAuthConfig {
                        auth_url: Url::parse("https://example.com/auth").unwrap(),
                        token_url: Url::parse("https://example.com/token").unwrap(),
                        client_id: goblin_domain::ClientId::from("client-id".to_string()),
                        scopes: vec![],
                        redirect_uri: None,
                        use_pkce: false,
    ...
    fn test_openai_responses_provider_new_with_oauth() {
        let provider = Provider {
            ...
            credential: Some(goblin_domain::AuthCredential {
                id: ProviderId::OPENAI,
                auth_details: goblin_domain::AuthDetails::OAuth {
                    tokens: goblin_domain::OAuthTokens::new(
                        "access-token",
                        None::<String>,
                        chrono::Utc::now() + chrono::Duration::hours(1),
    ...
    fn test_get_headers_with_oauth_device_custom_headers() {
        let provider = Provider {
            ...
            custom_headers: None,
            auth_methods: vec![goblin_domain::AuthMethod::OAuthDevice(
                goblin_domain::OAuthConfig {
                    auth_url: Url::parse("https://example.com/auth").unwrap(),
                    token_url: Url::parse("https://example.com/token").unwrap(),
                    client_id: goblin_domain::ClientId::from("client-id".to_string()),
                    scopes: vec![],
    ...
    fn test_get_headers_with_oauth_code_custom_headers() {
        let provider = Provider {
            ...
            credential: make_credential(ProviderId::OPENAI, "test-key"),
            custom_headers: None,
            auth_methods: vec![goblin_domain::AuthMethod::OAuthCode(
                goblin_domain::OAuthConfig {
                    auth_url: Url::parse("https://example.com/auth").unwrap(),
                    token_url: Url::parse("https://example.com/token").unwrap(),
                    client_id: goblin_domain::ClientId::from("client-id".to_string()),
    ...
    fn test_get_headers_with_codex_device_custom_headers() {
        let provider = Provider {
            ...
            custom_headers: None,
            auth_methods: vec![goblin_domain::AuthMethod::CodexDevice(
                goblin_domain::OAuthConfig {
                    auth_url: Url::parse(
                    ...
                    client_id: goblin_domain::ClientId::from(
                    ...
                    token_refresh_url: None,
                    custom_headers: Some(
                        [("originator".to_string(), "goblin".to_string())]
                            .into_iter()
                            .collect(),
                    ),
                    extra_auth_params: None,
    ...
    fn test_get_headers_codex_includes_chatgpt_account_id() {
        ...
        url_params.insert(
            goblin_domain::URLParam::from("chatgpt_account_id".to_string()),
            goblin_domain::URLParamValue::from("acct_test_123".to_string()),
        );
            ...
            credential: Some(goblin_domain::AuthCredential {
                ...
                auth_details: goblin_domain::AuthDetails::OAuth {
                    ...
                    config: goblin_domain::OAuthConfig {
                        ...
                        .unwrap(),
                        token_url: Url::parse("https://auth.openai.com/oauth/token").unwrap(),
                        client_id: goblin_domain::ClientId::from("app_test".to_string()),
                        scopes: vec![],
                        redirect_uri: None,
                        use_pkce: false,
                        token_refresh_url: None,
    ...
    fn test_get_headers_non_codex_does_not_include_chatgpt_account_id() {
        ...
        url_params.insert(
            goblin_domain::URLParam::from("chatgpt_account_id".to_string()),
            goblin_domain::URLParamValue::from("acct_should_not_appear".to_string()),
        );
