...
impl BedrockProvider {
    ...
    pub fn new(provider: Provider<Url>) -> Result<Self> {
        ...
        let region_param: goblin_domain::URLParam = "AWS_REGION".to_string().into();
    ...
    pub async fn chat(
        ...
        let output = self
            ...
            .map_err(|sdk_error| {
                ...
                if is_retryable {
                    goblin_domain::Error::Retryable(anyhow::anyhow!("{}", source)).into()
                } else {
            ...
            match event_stream.recv().await {
                ...
                Err(stream_error) => {
                    ...
                    let error = if is_retryable {
                        goblin_domain::Error::Retryable(anyhow::anyhow!(
    ...
    pub async fn models(&self) -> Result<Vec<Model>> {
        ...
        match &self.provider.models {
            Some(goblin_domain::ModelSource::Hardcoded(models)) => Ok(models.clone()),
            _ => Ok(vec![]),
        }
    }
}
...
impl IntoDomain for aws_sdk_bedrockruntime::types::ConverseStreamOutput {
    type Domain = goblin_domain::ChatCompletionMessage;
        ...
        match self {
                            ...
                            match reasoning {
                                aws_sdk_bedrockruntime::types::ReasoningContentBlockDelta::Text(
                                    ...
                                    ChatCompletionMessage::default()
                                        ...
                                        .add_reasoning_detail(goblin_domain::Reasoning::Part(vec![
                                            goblin_domain::ReasoningPart {
                                                text: Some(text),
                                                signature: None,
                                                ..Default::default()
                                            },
                                ...
                                aws_sdk_bedrockruntime::types::ReasoningContentBlockDelta::Signature(
                                    ...
                                    ChatCompletionMessage::default().add_reasoning_detail(
                                        goblin_domain::Reasoning::Part(vec![
                                            goblin_domain::ReasoningPart {
                                                text: None,
                                                signature: Some(sig),
                                                ..Default::default()
            ...
            ConverseStreamOutput::Metadata(metadata) => {
                ...
                let usage = metadata.usage.map(|u| {
                    ...
                    goblin_domain::Usage {
                        prompt_tokens: goblin_domain::TokenCount::Actual(u.total_tokens as usize),
                        completion_tokens: goblin_domain::TokenCount::Actual(
                            u.output_tokens as usize,
                        ),
                        total_tokens: goblin_domain::TokenCount::Actual(u.total_tokens as usize),
                        cached_tokens: goblin_domain::TokenCount::Actual(cached_tokens as usize),
                        ..Default::default()
                    }
                });
...
impl FromDomain<goblin_domain::Context>
    for aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput
{
    fn from_domain(context: goblin_domain::Context) -> anyhow::Result<Self> {
        ...
        let system: Vec<SystemContentBlock> = context
            ...
            .filter_map(|msg| match &msg.message {
                goblin_domain::ContextMessage::Text(text_msg)
                    if text_msg.has_role(goblin_domain::Role::System) =>
                {
        ...
        let messages: Vec<Message> = {
            ...
            let mut pending_tool_results: Vec<goblin_domain::ContextMessage> = Vec::new();

            for message in context.messages.into_iter() {
                if message.has_role(goblin_domain::Role::System) {
                    continue;
                }

                match &message.message {
                    goblin_domain::ContextMessage::Tool(_) => {
        ...
        let tool_config = if !context.tools.is_empty() {
            ...
            let choice = context
                .tool_choice
                .filter(|c| !matches!(c, goblin_domain::ToolChoice::None))
        ...
        let adjusted_top_p = if has_thinking {
            ...
            context.top_p.map(|p| {
                let value = p.value();
                if value < 0.95 {
                    goblin_domain::TopP::new(0.95).unwrap()
                } else {
                    p
                }
            })
...
impl FromDomain<Vec<goblin_domain::ContextMessage>> for aws_sdk_bedrockruntime::types::Message {
    fn from_domain(tool_results: Vec<goblin_domain::ContextMessage>) -> anyhow::Result<Self> {
        ...
        for msg in tool_results {
            match msg {
                goblin_domain::ContextMessage::Tool(tool_result) => {
...
impl FromDomain<goblin_domain::ContextMessage> for aws_sdk_bedrockruntime::types::Message {
    fn from_domain(msg: goblin_domain::ContextMessage) -> anyhow::Result<Self> {
        ...
        match msg {
            goblin_domain::ContextMessage::Text(text_msg) => {
                ...
                if text_msg.role == goblin_domain::Role::Assistant
                ...
                let role = match text_msg.role {
                    goblin_domain::Role::User => ConversationRole::User,
                    goblin_domain::Role::Assistant => ConversationRole::Assistant,
                    goblin_domain::Role::System => {
            ...
            goblin_domain::ContextMessage::Image(img) => {
...
impl FromDomain<goblin_domain::ToolCallArguments> for aws_smithy_types::Document {
    fn from_domain(args: goblin_domain::ToolCallArguments) -> anyhow::Result<Self> {
        use anyhow::Context as _;
...
impl FromDomain<goblin_domain::ToolDefinition> for aws_sdk_bedrockruntime::types::Tool {
    fn from_domain(tool: goblin_domain::ToolDefinition) -> anyhow::Result<Self> {
...
impl FromDomain<goblin_domain::ToolChoice> for aws_sdk_bedrockruntime::types::ToolChoice {
    fn from_domain(choice: goblin_domain::ToolChoice) -> anyhow::Result<Self> {
        use aws_sdk_bedrockruntime::types::{
            AnyToolChoice, AutoToolChoice, SpecificToolChoice, ToolChoice,
        };

        let bedrock_choice = match choice {
            goblin_domain::ToolChoice::Auto => ToolChoice::Auto(AutoToolChoice::builder().build()),
            goblin_domain::ToolChoice::Required => ToolChoice::Any(AnyToolChoice::builder().build()),
            goblin_domain::ToolChoice::Call(tool_name) => ToolChoice::Tool(
                SpecificToolChoice::builder()
                    .name(tool_name.to_string())
                    .build()
                    .map_err(|e| anyhow::anyhow!("Failed to build tool choice: {}", e))?,
            ),
            goblin_domain::ToolChoice::None => {
...
#[cfg(test)]
mod tests {
    use goblin_domain::InputModality;
    ...
    async fn test_models_returns_hardcoded() {
        use goblin_domain::{Model, ModelId, ModelSource};
    ...
    fn test_into_domain_content_block_delta_text() {
        ...
        use goblin_domain::{ChatCompletionMessage, Content};
    ...
    fn test_into_domain_content_block_start_tool_use() {
        ...
        use goblin_domain::{ChatCompletionMessage, Content, ToolCallId, ToolCallPart, ToolName};
    ...
    fn test_into_domain_message_stop_end_turn() {
        ...
        use goblin_domain::{ChatCompletionMessage, Content, FinishReason};
    ...
    fn test_into_domain_metadata_with_usage() {
        use aws_sdk_bedrockruntime::types::ConverseStreamOutput;
        use goblin_domain::{ChatCompletionMessage, Content, TokenCount};
        ...
        let actual = fixture.into_domain();
        let expected =
            ChatCompletionMessage::assistant(Content::part("")).usage(goblin_domain::Usage {
    ...
    fn test_from_domain_tool_choice_auto() {
        ...
        let fixture = goblin_domain::ToolChoice::Auto;
    ...
    fn test_from_domain_tool_choice_required() {
        ...
        let fixture = goblin_domain::ToolChoice::Required;
        let actual = ToolChoice::from_domain(fixture).unwrap();
    ...
    fn test_from_domain_tool_choice_call() {
        ...
        let fixture = goblin_domain::ToolChoice::Call(goblin_domain::ToolName::new("my_tool"));
        let actual = ToolChoice::from_domain(fixture).unwrap();
    ...
    fn test_from_domain_tool_definition() {
        ...
        use goblin_domain::ToolDefinition;
        ...
        let fixture = ToolDefinition {
            name: goblin_domain::ToolName::new("test_tool"),
    ...
    fn test_from_domain_context_message_text_user() {
        ...
        use goblin_domain::{ContextMessage, Role, TextMessage};
    ...
    fn test_from_domain_context_message_text_assistant() {
        use aws_sdk_bedrockruntime::types::{ContentBlock, ConversationRole, Message};
        use goblin_domain::{ContextMessage, TextMessage};
    ...
    fn test_from_domain_context_message_text_assistant_reasoning_uses_message_signature() {
        ...
        use goblin_domain::{ContextMessage, ReasoningFull, TextMessage};
        use pretty_assertions::assert_eq;
    ...
    fn test_from_domain_context_message_tool_result() {
        ...
        use goblin_domain::{ContextMessage, ToolCallId, ToolResult};
    ...
    fn test_from_domain_context_with_system_messages() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use goblin_domain::{Context, ContextMessage, Role, TextMessage};
    ...
    fn test_from_domain_context_with_temperature() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use goblin_domain::{Context, Temperature};
    ...
    fn test_from_domain_context_with_reasoning_adjusts_top_p() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use goblin_domain::{Context, ReasoningConfig, TopP};
    ...
    fn test_from_domain_context_with_reasoning_enabled() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use goblin_domain::{Context, ReasoningConfig};
