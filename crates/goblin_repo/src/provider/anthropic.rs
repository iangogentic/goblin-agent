...
use goblin_domain::{ChatRepository, Provider, ProviderId};
    ...
    fn get_headers(&self) -> Vec<(String, String)> {
        ...
        let api_key = self
            ...
            .map(|c| match &c.auth_details {
                ...
                goblin_domain::AuthDetails::GoogleAdc(api_key) => api_key.as_str(),
            });
...
impl<T: HttpInfra> Anthropic<T> {
    ...
    pub async fn models(&self) -> anyhow::Result<Vec<Model>> {
        ...
        match models {
            goblin_domain::ModelSource::Url(url) => {
            ...
            goblin_domain::ModelSource::Hardcoded(models) => {
...
mod tests {
    ...
    fn create_anthropic(base_url: &str) -> anyhow::Result<Anthropic<MockHttpClient>> {
        ...
        let provider = Provider {
            id: goblin_app::domain::ProviderId::ANTHROPIC,
            provider_type: goblin_domain::ProviderType::Llm,
            response: Some(goblin_app::domain::ProviderResponse::Anthropic),
            url: chat_url,
            ...
            url_params: vec![],
            models: Some(goblin_domain::ModelSource::Url(model_url)),
            custom_headers: None,
        };
    ...
    async fn test_url_for_models() {
        ...
        let provider = Provider {
            ...
            auth_methods: vec![goblin_domain::AuthMethod::ApiKey],
            url_params: vec![],
            models: Some(goblin_domain::ModelSource::Url(model_url.clone())),
            custom_headers: None,
        };
    ...
    fn test_vertex_ai_removes_output_format() {
        use goblin_domain::ResponseFormat;
        ...
        let provider = Provider {
            ...
            credential: Some(goblin_domain::AuthCredential {
                id: ProviderId::VERTEX_AI_ANTHROPIC,
                auth_details: goblin_domain::AuthDetails::GoogleAdc(goblin_domain::ApiKey::from(
                    "test-token".to_string(),
                )),
                url_params: std::collections::HashMap::new(),
            }),
            auth_methods: vec![goblin_domain::AuthMethod::GoogleAdc],
            url_params: vec![],
            models: Some(goblin_domain::ModelSource::Hardcoded(vec![])),
            custom_headers: None,
        };
...
impl<F: HttpInfra> AnthropicResponseRepository<F> {
    ...
    fn create_client(&self, provider: Provider<Url>) -> anyhow::Result<Anthropic<F>> {
        ...
        let is_oauth = provider
            .credential
            .as_ref()
            .map(|c| matches!(c.auth_details, goblin_domain::AuthDetails::OAuth { .. }))
            .unwrap_or(false);
