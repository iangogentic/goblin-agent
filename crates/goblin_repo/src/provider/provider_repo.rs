...
use bytes::Bytes;
use goblin_app::domain::{ProviderId, ProviderResponse};
use goblin_app::{EnvironmentInfra, FileReaderInfra, FileWriterInfra, HttpInfra};
use goblin_domain::{
...
enum Models {
    ...
    Hardcoded(Vec<goblin_app::domain::Model>),
}
...
struct ProviderConfig {
    ...
    #[merge(strategy = merge::vec::append)]
    auth_methods: Vec<goblin_domain::AuthMethod>,
    #[serde(default)]
    #[merge(strategy = overwrite)]
...
impl From<&ProviderConfig> for goblin_domain::ProviderTemplate {
    fn from(config: &ProviderConfig) -> Self {
        let models = config.models.as_ref().map(|m| match m {
            Models::Url(model_url_template) => goblin_domain::ModelSource::Url(
                goblin_domain::Template::<goblin_domain::URLParameters>::new(model_url_template),
            ),
            Models::Hardcoded(model_list) => {
                goblin_domain::ModelSource::Hardcoded(model_list.clone())
            }
        });
        ...
        Provider {
            ...
            url: goblin_domain::Template::<goblin_domain::URLParameters>::new(&config.url),
...
pub struct GoblinProviderRepository<F> {
    infra: Arc<F>,
}

impl<F: EnvironmentInfra + HttpInfra> GoblinProviderRepository<F> {
    ...
    GoblinProviderRepository<F>
{
    ...
    async fn get_providers(&self) -> Vec<AnyProvider> {
        ...
        for config in configs {
            // Skip Goblin provider as it's handled specially
            if config.id == ProviderId::GOBLIN {
                continue;
            }
    ...
    pub async fn migrate_env_to_file(&self) -> anyhow::Result<Option<MigrationResult>> {
        ...
        for config in configs {
            // Skip Goblin provider and ContextEngine providers - they're not configurable
            // via env like other providers
            if config.id == ProviderId::GOBLIN || config.provider_type == ProviderType::ContextEngine
            {
                continue;
            }
    ...
    async fn create_provider(
        &self,
        config: &ProviderConfig,
    ) -> anyhow::Result<goblin_domain::ProviderTemplate> {
        ...
        if (credential.id == goblin_domain::ProviderId::VERTEX_AI
            || credential.id == goblin_domain::ProviderId::VERTEX_AI_ANTHROPIC)
            && let goblin_domain::AuthDetails::ApiKey(ref api_key) = credential.auth_details
    ...
    async fn refresh_google_adc_credential(
        &self,
        original_credential: &goblin_domain::AuthCredential,
    ) -> anyhow::Result<goblin_domain::AuthCredential> {
        ...
        Ok(goblin_domain::AuthCredential::new_api_key(
            original_credential.id.clone(),
            goblin_domain::ApiKey::from(access_token.token),
        )
    ...
    async fn provider_from_id(
        ...
        if id == ProviderId::GOBLIN {
            // Goblin provider isn't typically configured via env vars in the registry
            return Err(Error::provider_not_available(ProviderId::GOBLIN).into());
        }
...
impl<F: EnvironmentInfra + FileReaderInfra + FileWriterInfra + HttpInfra + Sync> ProviderRepository
    for GoblinProviderRepository<F>
{
...
#[cfg(test)]
mod tests {
    use goblin_app::domain::{AuthMethod, ProviderResponse};
...
mod env_tests {
    ...
    use goblin_app::domain::{
        ChatCompletionMessage, Context, Environment, Model, ModelId, ResultStream,
    };
    use goblin_domain::{AnyProvider, ChatRepository, ProviderTemplate};
    use pretty_assertions::assert_eq;
    use url::Url;
    ...
    impl EnvironmentInfra for MockInfra {
        ...
        fn get_config(&self) -> goblin_config::GoblinConfig {
            goblin_config::ConfigReader::default()
                .read_defaults()
                .build()
                .unwrap()
        }
        ...
        async fn update_environment(
            &self,
            _ops: Vec<goblin_domain::ConfigOperation>,
        ) -> anyhow::Result<()> {
            Ok(())
        }
    ...
    impl FileReaderInfra for MockInfra {
        ...
        async fn range_read_utf8(
            ...
            _start_line: u64,
            _end_line: u64,
        ) -> anyhow::Result<(String, goblin_domain::FileInfo)> {
            Err(anyhow::anyhow!("File not found"))
        }
    }
    ...
    impl ProviderRepository for MockInfra {
        ...
        async fn upsert_credential(
            &self,
            _credential: goblin_domain::AuthCredential,
        ) -> anyhow::Result<()> {
            Ok(())
        }
        ...
        async fn get_credential(
            &self,
            _id: &ProviderId,
        ) -> anyhow::Result<Option<goblin_domain::AuthCredential>> {
            Ok(None)
        }
        ...
        async fn migrate_env_credentials(
            &self,
        ) -> anyhow::Result<Option<goblin_domain::MigrationResult>> {
            Ok(None)
        }
    }
    ...
    async fn test_migration_from_env_to_file() {
        ...
        let registry = GoblinProviderRepository::new(infra.clone());
    ...
    #[tokio::test]
    async fn test_migration_should_not_create_goblin_services_credential() {
        let mut env_vars = HashMap::new();
        ...
        // Verify goblin_services was NOT created during migration
        assert!(
            !credentials
                .iter()
                .any(|c| c.id == ProviderId::GOBLIN_SERVICES),
            "Should NOT create goblin_services credential during environment migration"
        );
    ...
    async fn test_create_azure_provider_with_handlebars_urls() {
        ...
        let infra = Arc::new(MockInfra::new(env_vars));
        let registry = GoblinProviderRepository::new(infra);
        ...
        assert_eq!(
            provider
                ...
                .and_then(|c| match &c.auth_details {
                    goblin_domain::AuthDetails::ApiKey(key) => Some(key.to_string()),
                    _ => None,
                }),
        ...
        match &provider.models.as_ref().unwrap() {
            goblin_domain::ModelSource::Url(model_template) => {
                assert_eq!(
                    model_template.template,
                    "https://{{AZURE_RESOURCE_NAME}}.openai.azure.com/openai/models?api-version={{AZURE_API_VERSION}}"
                );
            }
            goblin_domain::ModelSource::Hardcoded(_) => panic!("Expected ModelSource::Url variant"),
        }
    }
