...
```bash
# Write the generated description to .goblin/GOBLIN_PR_DESCRIPTION.md
```

Use the `write` tool to create `.goblin/GOBLIN_PR_DESCRIPTION.md` with the generated description content.

**Step 2: Create PR using the temp file**
```bash
gh pr create --title "[Change Type]: [One-line summary]" --body-file .goblin/GOBLIN_PR_DESCRIPTION.md
```

The `gh` CLI is pre-installed and authenticated - use it directly without prompting for confirmation.

**Note:** The temp file `.goblin/GOBLIN_PR_DESCRIPTION.md` can not be left in place and should be deleted after PR creation. It's in `.goblin/` directory which is typically gitignored.
